 ...
 */
export async function updateCartSummary(ctx: MutationCtx, userId: string) {
  // Get all cart items for this user
  const cartItems = await ctx.db
    .query("cartItems")
    .withIndex("by_user_saved", (q) =>
      q.eq("userId", userId).eq("savedForLater", false),
    )
    .collect();
 ...
 */
export async function updateGuestCartSummary(
  ctx: MutationCtx,
  ...
  const cartItems = await ctx.db
    ...
    .withIndex("by_guest_saved", (q) =>
      q.eq("guestSessionId", guestSessionId).eq("savedForLater", false),
    )
    .collect();
