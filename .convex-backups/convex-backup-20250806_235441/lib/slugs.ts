 ...
 */
export function truncateSlug(slug: string, maxLength = 200): string {
...
export async function generateUniqueSlug(
  ...
  try {
    ...
    if (!sampleRecord || !("slug" in sampleRecord)) {
      console.warn(
        `Table ${tableName} does not appear to have a slug field. Using slug "${slug}" without uniqueness check.`,
      );
      return slug;
    }
  } catch {
    ...
    console.warn(
      `Unable to verify slug field for table ${tableName}. Using slug "${slug}" without uniqueness check.`,
    );
    return slug;
  }
  ...
  while (!isUnique) {
    try {
      ...
      const slugExists = existingEntities.some(
        (entity) => entityId === undefined || entity._id !== entityId,
      );
      ...
      console.error(
        `Error checking slug uniqueness for ${String(tableName)}: ${String(e)}`,
      );
      return slug;
    }
  }
