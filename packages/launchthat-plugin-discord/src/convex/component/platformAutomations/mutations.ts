...
export const createAutomation = mutation({
  ...
  returns: v.id("platformDiscordAutomations"),
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("platformDiscordAutomations", {
      guildId,
      name,
...
export const updateAutomation = mutation({
  args: {
    automationId: v.id("platformDiscordAutomations"),
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    const nextEnabled =
      typeof args.enabled === "boolean" ? args.enabled : Boolean((row as any).enabled);
    const nextTrigger =
      args.trigger ??
    ...
    await ctx.db.patch(args.automationId, {
      ...
      conditions:
        typeof args.conditions === "undefined" ? (row as any).conditions : args.conditions,
      action: args.action ?? (row as any).action,
      state: nextEnabled
        ? {
            ...(typeof (row as any).state === "object" && (row as any).state
              ? (row as any).state
              : {}),
            nextRunAt: typeof nextRunAt === "number" ? nextRunAt : undefined,
          }
...
export const markAutomationRun = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const state =
      prevState && typeof prevState === "object"
        ? {
            ...prevState,
            lastRunAt:
              typeof args.lastRunAt === "number" ? args.lastRunAt : prevState.lastRunAt,
            cursor: typeof args.cursor === "string" ? args.cursor : prevState.cursor,
            nextRunAt:
              typeof args.nextRunAt === "number" ? args.nextRunAt : prevState.nextRunAt,
          }
        : {
    ...
    await ctx.db.patch(args.automationId, {
      state,
      nextRunAt:
        typeof args.nextRunAt === "number" ? args.nextRunAt : (row as any).nextRunAt,
      updatedAt: Date.now(),
    });
    return null;
  },
