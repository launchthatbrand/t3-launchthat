...
export function OrderForm({
  ...
  const handleInputChange = (field: string, value: string) => {
    setFormData((prev) => {
      ...
      if (keys.length === 1) {
        return { ...prev, [keys[0] as keyof OrderFormData]: value };
        ...
        if (firstKey === "shippingAddress" || firstKey === "billingAddress") {
          return {
            ...
            [firstKey]: {
              ...prev[firstKey],
              [secondKey as keyof AddressData]: value,
            },
          };
        }
      }
  ...
  const handleAddProduct = (productId: Id<"products">, quantity = 1) => {
    if (!productsQuery) {
    ...
    const product = productsQuery.find(
      (item: Doc<"products">) => item._id === productId,
    );
  ...
  const lineItemColumns = useMemo<ColumnDefinition<OrderLineItem>[]>(() => {
    return [
      {
        id: "item",
        header: "Product",
        cell: (item: OrderLineItem) => (
            ...
            <CardContent>
              {formData.lineItems.length > 0 ? (
                <EntityList
                  ...
                  columns={lineItemColumns as ColumnDefinition<OrderLineItem>[]}
