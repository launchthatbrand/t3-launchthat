...
const Slide = ({ slide, index, current, handleSlideClick }: SlideProps) => {
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      <li
        ref={slideRef}
        className="relative z-10 mx-[1vmin] flex h-[40vmin] w-[60vmin] flex-1 flex-col items-center justify-center text-center text-white opacity-100 transition-all duration-300 ease-in-out"
...
export default function Carousel({ slides, className }: CarouselProps) {
  ...
  return (
    <div
      className={cn("relative mx-auto h-[40vmin] w-[60vmin]", className)}
      aria-labelledby={`carousel-heading-${id}`}
    >
      <ul
        className="absolute mx-[-1vmin] flex transition-transform duration-1000 ease-in-out"
        style={{
