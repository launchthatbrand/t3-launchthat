...
function generateCollectionJoinsSchemas(
...
function generateLocaleEntitySchemas(
...
function generateAuthEntitySchemas(
  entities: SanitizedCollectionConfig[],
): JSONSchema4 {
        ...
        switch (field.type) {
          ...
          case "blocks": {
            ...
            fieldSchema = {
              ...
              items: hasBlocks
                ? {
                    oneOf: (field.blockReferences ?? field.blocks).map(
                      (block) => {
                        if (typeof block === "string") {
                          const resolvedBlock = config?.blocks?.find(
                            (b) => b.slug === block,
                          );
                          return {
          ...
          case "richText": {
            ...
            if (typeof field.editor === "function") {
              throw new Error(
                "Attempted to access unsanitized rich text editor.",
              );
            }
          ...
          case "select": {
            ...
            const isTimezoneField =
              previousField?.type === "date" &&
              previousField.timezone &&
              field.name.includes("_tz");
...
export function entityToJSONSchema(
  ...
  const customIdField = mutableFields.find(
    (field) => field.name === "id",
...
export function authCollectionToOperationsJSONSchema(
  ...
  const loginUserFields: JSONSchema4 = generateAuthFieldTypes({
    type: "login",
    loginWithUsername,
  });
...
function generateAuthOperationSchemas(
  collections: SanitizedCollectionConfig[],
): JSONSchema4 {
...
export function configToJSONSchema(
  ...
  const timezoneDefinitions = timezonesToJSONSchema(
  ...
  const authOperationDefinitions = [...config.collections]
    ...
    .reduce(
      (acc, authCollection) => {
        acc.auth[authCollection.slug] =
          authCollectionToOperationsJSONSchema(authCollection);
        return acc;
      },
      { auth: {} },
    );
  ...
  let jsonSchema: JSONSchema4 = {
    ...
    properties: {
      ...
      collectionsJoins: generateCollectionJoinsSchemas(
        config.collections || [],
      ),
