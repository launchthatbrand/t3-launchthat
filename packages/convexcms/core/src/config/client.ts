...
export type UnsanitizedClientConfig = {
  admin: {
    ...
    SanitizedConfig["admin"],
    "components" | "dependencies" | "livePreview"
  >;
  blocks: ClientBlock[];
...
export const createClientConfig = ({
  ...
  for (const key in config) {
    ...
    switch (key) {
      case "admin":
        ...
        if (config.admin.livePreview) {
          ...
          if (config.admin.livePreview.breakpoints) {
            clientConfig.admin.livePreview.breakpoints =
      ...
      case "collections":
        (clientConfig.collections as ClientCollectionConfig[]) =
        ...
        break;
      case "globals":
        (clientConfig.globals as ClientGlobalConfig[]) =
      ...
      case "localization":
        if (typeof config.localization === "object" && config.localization) {
          ...
          if (config.localization.localeCodes) {
            clientConfig.localization.localeCodes =
          ...
          if (config.localization.locales) {
            ...
            for (const locale of config.localization.locales) {
              if (locale) {
                const clientLocale: Partial<
                  (typeof config.localization.locales)[0]
                > = {};
                if (locale.code) {
