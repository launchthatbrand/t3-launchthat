...
> =
  | false
  | RawPayloadComponent<TComponentServerProps, TComponentClientProps>
  | string;
...
export type ServerOnlyLivePreviewProperties = keyof Pick<
  LivePreviewConfig,
  "url"
>;
...
export type CustomComponent<
  TAdditionalProps extends object = Record<string, any>,
...
export type FetchAPIFileUploadOptions = {
   ...
   */
  limitHandler?:
    | ((args: { request: Request; size: number }) => void)
    | boolean
    | undefined;
  /**
...
export type Config = {
  ...
  // --- Comment out Payload specific structures for Phase 1 ---
  // TODO: CONVEX_REFACTOR_PHASE_3 - Replace/reintegrate with Convex logic
  // blocks?: Block[];
  // collections?: CollectionConfig[];
  // compatibility?: { /* ... */ };
  // db: DatabaseAdapterResult; // Replaced by Convex
  // email?: EmailAdapter | Promise<EmailAdapter>;
  // endpoints?: Endpoint[];
  // globals?: GlobalConfig[];
  // graphQL?: { /* ... */ };
  // hooks?: { afterError?: AfterErrorHook[]; };
  // jobs?: JobsConfig;
  // plugins?: Plugin[];
  // queryPresets?: { /* ... */ };
  ...
  /** A whitelist array of URLs to allow Payload cookies to be accepted from */
  csrf?: string[];
  /** Extension point to add your custom data. Server only. */
  custom?: Record<string, any>;
  /** i18n config settings */
  i18n?: I18nOptions<{} | DefaultTranslationsObject>;
  /** Automatically index all sortable top-level fields */
  ...
  logger?:
    | "sync"
    | { destination?: DestinationStream; options: pino.LoggerOptions }
    | PayloadLogger;
  /** Override the log level of errors */
  loggingLevels?: Partial<Record<ErrorName, false | Level>>;
  /** The maximum allowed depth */
  maxDepth?: number;
  /** A function that is called immediately following startup */
  onInit?: (payload: Payload) => Promise<void> | void;
  ...
  secret: string;
  /** Define the absolute URL of your app */
  serverURL?: string;
  /** Pass in a local copy of Sharp */
  sharp?: SharpDependency;
  /** Send anonymous telemetry data */
  telemetry?: boolean;
  /** Control how typescript interfaces are generated */
  typescript?: {
    /**
...
export type SanitizedConfig = {
  ...
  // collections: SanitizedCollectionConfig[]; // Use generated AdminConfig instead
  // endpoints: Endpoint[];
  // globals: SanitizedGlobalConfig[];
  // jobs: JobsConfig; // Redefine here, as the DeepRequired<Config> can break its type
  // --- End commented out sections ---

  // Keep other sanitized properties
  editor?: RichTextAdapter<any, any, any>;
  // ... other properties from Omit<DeepRequired<Config>, ... > ...
} & Omit<
  DeepRequired<Config>,
  // Add commented-out keys here as well to match Config
  | "admin"
  | "i18n"
  | "localization"
  | "upload"
  // TODO: CONVEX_REFACTOR_PHASE_3 - Add keys corresponding to commented-out Config props
  | "blocks"
  | "collections"
  | "db"
  | "email"
  ...
  | "plugins"
  | "queryPresets"
  // Keep properties that were NOT commented out in Config
  | "auth"
  | "bin"
  | "compatibility" // Keep or comment based on need
  | "cookiePrefix"
  | "cors"
  | "csrf"
  | "custom"
...
// ----- ADD MISSING ADMIN CONFIG TYPES -----

export interface AdminCollectionField {
  label: string;
  type: string; // 'string', 'number', 'boolean', 'id', 'array', 'object', etc.
  // Add other common field properties here if needed (e.g., required, admin config)
  // required?: boolean;
  // admin?: { ... }
}
...
export interface AdminCollectionConfig {
  ...
  // Add other common collection config properties here if needed (e.g., admin config)
  // admin?: { ... }
}

export interface AdminConfig {
  collections: Record<string, AdminCollectionConfig>;
  // Add other top-level config properties here if needed
}
