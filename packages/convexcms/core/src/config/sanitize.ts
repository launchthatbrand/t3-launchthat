...
} from "../query-presets/config.js";
import {
  getDefaultJobsCollection,
  jobsCollectionSlug,
} from "../queues/config/index.js";
import { flattenBlock } from "../utilities/flattenAllFields.js";
import { getSchedulePublishTask } from "../versions/schedule/job.js";
import { addDefaultsToConfig } from "./defaults.js";
import { setupOrderable } from "./orderable/index.js";

const sanitizeAdminConfig = (
  configToSanitize: Config,
): Partial<SanitizedConfig> => {
  ...
  if (sanitizedConfig?.admin?.timezones) {
    if (
      typeof sanitizedConfig?.admin?.timezones?.supportedTimezones ===
      "function"
    ) {
  ...
  (sanitizedConfig.admin.timezones.supportedTimezones as Timezone[]).forEach(
    (timezone) => {
...
export const sanitizeConfig = async (
  ...
  const config: Partial<SanitizedConfig> =
    sanitizeAdminConfig(configWithDefaults);
  ...
  if (config.localization && config.localization.locales?.length > 0) {
    ...
    if (typeof firstLocale === "string") {
      ...
      config.localization.localeCodes = config.localization.locales.map(
        (locale) => locale.code,
      );
  ...
  if (incomingConfig?.i18n) {
    ...
    const supportedLangKeys = <AcceptedLanguages[]>(
      Object.keys(i18nConfig.supportedLanguages)
    );
    const fallbackLang =
      incomingConfig.i18n?.fallbackLanguage || i18nConfig.fallbackLanguage;
    ...
    i18nConfig.translations =
      (incomingConfig.i18n
        ?.translations as SanitizedConfig["i18n"]["translations"]) ||
      i18nConfig.translations;
  }

  config.i18n = i18nConfig;

  const richTextSanitizationPromises: Array<
    (config: SanitizedConfig) => Promise<void>
  > = [];
  ...
  if (
    schedulePublishCollections.length > 0 ||
    schedulePublishGlobals.length > 0
  ) {
  ...
  if (
    (Array.isArray(configWithDefaults.jobs?.tasks) &&
      configWithDefaults.jobs?.tasks?.length) ||
    (Array.isArray(configWithDefaults.jobs?.workflows) &&
      configWithDefaults.jobs?.workflows?.length)
  ) {
    let defaultJobsCollection = getDefaultJobsCollection(
      config as unknown as Config,
    );

    if (defaultJobsCollection) {
      if (
        typeof configWithDefaults.jobs.jobsCollectionOverrides === "function"
      ) {
        ...
        if (hooks && config?.jobs?.runHooks !== true) {
          for (const hook of Object.keys(hooks)) {
            const defaultAmount =
              hook === "afterRead" || hook === "beforeChange" ? 1 : 0;
