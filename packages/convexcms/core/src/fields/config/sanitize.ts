...
type Args = {
   ...
   */
  richTextSanitizationPromises?: Array<
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    if (fieldAffectsData(field)) {
      ...
      if (typeof field.localized !== "undefined") {
        ...
        if (
          process.env
            .NEXT_PUBLIC_PAYLOAD_COMPATIBILITY_allowLocalizedWithinLocalized !==
            "true" &&
          parentIsLocalized &&
    ...
    if (field.type === "blocks" && field.blocks) {
      if (field.blockReferences && field.blocks?.length) {
        throw new Error(
          "You cannot have both blockReferences and blocks in the same blocks field",
        );
      }
    ...
    if ("fields" in field && field.fields) {
      field.fields = await sanitizeFields({
        config,
        existingFieldNames: fieldAffectsData(field)
          ? new Set()
          : existingFieldNames,
        fields: field.fields,
    ...
    if (field.type === "tabs") {
      for (let j = 0; j < field.tabs.length; j++) {
        ...
        tab.fields = await sanitizeFields({
          ...
          fields: tab.fields,
          joinPath: tabHasName(tab)
            ? `${joinPath ? joinPath + "." : ""}${tab.name}`
            : joinPath,
          joins,
          parentIsLocalized:
            parentIsLocalized || (tabHasName(tab) && tab.localized),
          polymorphicJoins,
          requireFieldLevelRichTextEditor,
          richTextSanitizationPromises,
          validRelationships,
    ...
    if (
      field.type === "ui" &&
      typeof field.admin.disableBulkEdit === "undefined"
    ) {
