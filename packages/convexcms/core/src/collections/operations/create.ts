...
>(
  ...
  try {
    const shouldCommit =
      !args.disableTransaction && (await initTransaction(args.req));
    ...
    if (duplicateFromID) {
      ...
      duplicatedFromDocWithLocales =
        duplicateResult.duplicatedFromDocWithLocales;
    }
    ...
    const { data: newFileData, files: filesToUpload } = await generateFileData({
      ...
      throwOnMissingFile:
        !shouldSaveDraft &&
        collection.config.upload.filesRequiredOnCreate !== false,
    });
    ...
    if (collectionConfig.auth && !collectionConfig.auth.disableLocalStrategy) {
      if (collectionConfig.auth.verify) {
        resultWithLocales._verified =
          Boolean(resultWithLocales._verified) || false;
        resultWithLocales._verificationToken = crypto
          .randomBytes(20)
          .toString("hex");
      }
