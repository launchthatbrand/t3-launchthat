    ...
    switch (key) {
            ...
            case "description":
              if (
                ...
                if (collection.admin.description) {
                  clientCollection.admin.description =
      ...
      case "auth":
        ...
        if (collection.auth.disableLocalStrategy) {
          clientCollection.auth.disableLocalStrategy =
        ...
        if (collection.auth.loginWithUsername) {
          clientCollection.auth.loginWithUsername =
            collection.auth.loginWithUsername;
        }
        if (collection.auth.maxLoginAttempts !== undefined) {
          // Check for undefined as it can be a number (0)
          clientCollection.auth.maxLoginAttempts =
            collection.auth.maxLoginAttempts;
        }
        if (collection.auth.removeTokenFromResponses) {
          clientCollection.auth.removeTokenFromResponses =
            collection.auth.removeTokenFromResponses;
        }
        ...
        if (collection.auth.tokenExpiration) {
          clientCollection.auth.tokenExpiration =
      ...
      case "upload":
        ...
        for (const uploadKey in collection.upload) {
          ...
          if (uploadKey === "imageSizes") {
            clientCollection.upload.imageSizes =
