 ...
 */
function getAdjustedComponentPath(
...
export function addPayloadComponentToImportMap({
  ...
  const { exportName, path: componentPath } =
    parsePayloadComponent(payloadComponent);
  ...
  const importIdentifier =
    exportName +
    "_" +
    crypto.createHash("md5").update(componentPath).digest("hex");
  ...
  if (isRelativePath) {
    const adjustedComponentPath = getAdjustedComponentPath(
      importMapToBaseDirPath,
      componentPath,
    );
