...
export function generateJobsJSONSchemas(
  ...
  if (jobsConfig?.tasks?.length) {
    for (const task of jobsConfig.tasks) {
      ...
      const normalizedTaskSlug =
        task.slug[0].toUpperCase() + task.slug.slice(1);

      definitions.set(
        task.interfaceName ?? `Task${normalizedTaskSlug}`,
        fullTaskJsonSchema,
      );
    }
  ...
  if (jobsConfig?.workflows?.length) {
    for (const workflow of jobsConfig.workflows) {
      ...
      const normalizedWorkflowSlug =
        workflow.slug[0].toUpperCase() + workflow.slug.slice(1);
