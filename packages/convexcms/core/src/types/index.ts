...
export type ReplaceAny<T, DefaultType> =
  IsAny<T> extends true ? DefaultType : T;
...
export type ApplyDisableErrors<
  T,
  DisableErrors = false,
> = false extends DisableErrors ? T : null | T;
  ...
  : string extends keyof Select
    ...
    : // START Handle types when they aren't generated
      ...
      ? Select extends SelectIncludeType
        ? {
            [K in Data extends TypeWithID
              ? "id" | keyof Select
              : keyof Select]: K extends "id" ? number | string : unknown;
          }
        : Data
