...
import { buildVersionGlobalFields, toWords } from "@convexcms/core";
import {
  GraphQLBoolean,
  GraphQLInt,
  GraphQLNonNull,
...
export function initGlobals({
  ...
  Object.keys(graphqlResult.globals.config).forEach((slug) => {
    ...
    const formattedName = graphQL?.name
      ? graphQL.name
      : singular(toWords(global.slug, true));
    ...
    const queriesEnabled =
      typeof global.graphQL !== "object" || !global.graphQL.disableQueries;
    const mutationsEnabled =
      typeof global.graphQL !== "object" || !global.graphQL.disableMutations;
    ...
    if (global.versions) {
      const idType =
        config.db.defaultIDType === "number" ? GraphQLInt : GraphQLString;
      ...
      if (mutationsEnabled) {
        graphqlResult.Mutation.fields[
          `restoreVersion${formatName(formattedName)}`
        ] = {
          type: graphqlResult.globals.graphQL[slug].type,
          args: {
            id: { type: idType },
