...
import { fieldAffectsData, getLoginOptions } from "@convexcms/core/shared";
import {
  GraphQLBoolean,
  GraphQLInt,
  GraphQLNonNull,
...
import {
  ...
  getCollectionIDType,
} from "./buildMutationInputType.js";
...
export function initCollections({
  config,
  graphqlResult,
}: InitCollectionsGraphQLArgs): void {
  Object.keys(graphqlResult.collections).forEach((slug) => {
    ...
    const queriesEnabled =
      typeof collectionConfig.graphQL !== "object" ||
      !collectionConfig.graphQL.disableQueries;
    const mutationsEnabled =
      typeof collectionConfig.graphQL !== "object" ||
      !collectionConfig.graphQL.disableMutations;
    ...
    if (collectionConfig.versions) {
      const versionIDType =
        config.db.defaultIDType === "text" ? GraphQLString : GraphQLInt;
      ...
      if (mutationsEnabled) {
        graphqlResult.Mutation.fields[
          `restoreVersion${formatName(singularName)}`
        ] = {
          type: collection.graphQL.type,
          args: {
            id: { type: versionIDType },
    ...
    if (collectionConfig.auth) {
      ...
      collection.graphQL.JWT = buildObjectType({
        ...
        config,
        fields: [
          ...collectionConfig.fields.filter(
