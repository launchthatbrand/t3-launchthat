...
function formattedNameResolver({
  field,
  ...rest
}: { field: Field } & GraphQLFieldConfig<any, any, any>): GraphQLFieldConfig<
  any,
  any,
  any
> {
...
type GenericFieldToSchemaMap = (
  args: { field: Field } & SharedArgs,
) => ObjectTypeConfig;
...
type FieldToSchemaMap = {
  ...
  collapsible: (
    args: { field: CollapsibleField } & SharedArgs,
  ) => ObjectTypeConfig;
  ...
  relationship: (
    args: { field: RelationshipField } & SharedArgs,
  ) => ObjectTypeConfig;
...
export const fieldToSchemaMap: FieldToSchemaMap = {
  array: ({
    ...
    const interfaceName =
      field?.interfaceName ||
      combineParentName(parentName, toWords(field.name, true));
  ...
  blocks: ({
    ...
    const blockTypes: GraphQLObjectType<any, any>[] = (
      ...
      if (!graphqlResult.types.blockTypes[blockSlug]) {
        ...
        const interfaceName =
          block?.interfaceName ||
          block?.graphQL?.singularName ||
          toWords(block.slug, true);
    ...
    const type = new GraphQLList(
      new GraphQLNonNull(
        new GraphQLUnionType({
          name: fullName,
          resolveType: (data) =>
            graphqlResult.types.blockTypes[data.blockType].name,
          types: blockTypes,
        }),
      ),
    ) as GraphQLOutputType;
  ...
  collapsible: ({
    ...
    field.fields.reduce((objectTypeConfigWithCollapsibleFields, subField) => {
      const addSubField: GenericFieldToSchemaMap =
        fieldToSchemaMap[subField.type];
      if (addSubField) {
        return addSubField({
          config,
          field: subField,
    ...
    const joinType = {
          ...
          docs: {
            type: new GraphQLNonNull(
              Array.isArray(field.collection)
                ...
                : new GraphQLList(
                    new GraphQLNonNull(
                      graphqlResult.collections[field.collection].graphQL.type,
                    ),
                  ),
            ),
          },
      ...
      args: {
        ...
        where: {
          type: Array.isArray(field.collection)
            ? GraphQLJSON
            : graphqlResult.collections[field.collection].graphQL
                .whereInputType,
        },
      },
      extensions: {
        complexity:
          typeof field?.graphQL?.complexity === "number"
            ? field.graphQL.complexity
            : 10,
      },
      async resolve(parent, args, context: Context) {
        ...
        if (Array.isArray(collection)) {
          throw new Error(
            "GraphQL with array of join.field.collection is not implemented",
          );
        }
  ...
  relationship: ({
    ...
    const relationshipName = combineParentName(
      parentName,
    ...
    if (Array.isArray(relationTo)) {
      relationToType = new GraphQLEnumType({
        ...
        values: relationTo
          .filter((relation) =>
            graphQLCollections.some(
              (collection) => collection.slug === relation,
            ),
          )
          .reduce(
            (relations, relation) => ({
        ...
        fields: {
          ...
          value: {
            type: new GraphQLUnionType({
              ...
              resolveType(data) {
                return graphqlResult.collections[data.collection].graphQL.type
                  .name;
              },
              types,
            }) as GraphQLOutputType,
    ...
    const relationsUseDrafts = (
      ...
      .some(
        (relation) =>
          graphqlResult.collections[relation].config.versions?.drafts,
      );
    ...
    const relationship: GraphQLFieldConfig<any, any, any> = {
      ...
      async resolve(parent, args, context: Context) {
        ...
        const fallbackLocale =
          args.fallbackLocale || context.req.fallbackLocale;
        ...
        if (hasManyValues) {
          ...
          const createPopulationPromise = async (relatedDoc, i) => {
            ...
            const isValidGraphQLCollection = isRelatedToManyCollections
              ? graphQLCollections.some((collection) =>
                  collectionSlug.includes(collection.slug),
                )
              : graphQLCollections.some(
                  (collection) => collectionSlug === collection.slug,
                );
        ...
        if (id) {
          if (
            graphQLCollections.some(
              (collection) => collection.slug === relatedCollectionSlug,
            )
          ) {
    ...
    [formatName(field.name)]: {
      ...
      async resolve(parent, args, context: Context) {
        ...
        if (editor?.graphQLPopulationPromises) {
          ...
          const populateDepth =
            field?.maxDepth !== undefined && field?.maxDepth < depth
              ? field?.maxDepth
              : depth;
  ...
  tabs: ({
    ...
    field.tabs.reduce((tabSchema, tab) => {
      if (tabHasName(tab)) {
        const interfaceName =
          tab?.interfaceName ||
          combineParentName(parentName, toWords(tab.name, true));
  ...
  upload: ({
    ...
    if (Array.isArray(relationTo)) {
      ...
      const types = relationTo.map(
        (relation) => graphqlResult.collections[relation].graphQL.type,
      );
