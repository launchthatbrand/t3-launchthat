...
import { toWords } from "@convexcms/core";
...
export const buildEntityPolicy = (args: BuildEntityPolicy) => {
  ...
  operations.forEach((operation) => {
    const operationTypeName = toWords(
      `${name}-${operation}-${scope || "Access"}`,
      true,
    );
...
export function buildPolicyType(args: BuildPolicyType): GraphQLObjectType {
  ...
  const globalTypeName = formatName(
    `${global?.graphQL?.name || slug}${typeSuffix || ""}`,
  );
...
export function buildPoliciesType(config: SanitizedConfig): GraphQLObjectType {
  ...
  Object.values(config.collections).forEach(
