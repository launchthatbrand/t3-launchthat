...
export function PublishButton({
  ...
  const publishSpecificLocale = useCallback(locale => {
    ...
    const action = `${serverURL}${api}${globalSlug ? `/globals/${globalSlug}` : `/${collectionSlug}/${id ? `${"/" + id}` : ""}`}${params ? "?" + params : ""}`;
    void submit({
      action,
      overrides: {
        _status: "published"
  ...
  const activeLocale = localization && localization?.locales.find(locale => typeof locale === "string" ? locale === localeCode : locale.code === localeCode);
