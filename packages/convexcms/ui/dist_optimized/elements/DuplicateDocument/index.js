...
export const DuplicateDocument = ({
  ...
  const duplicate = useCallback(async () => {
    setRenderModal(true);
    await requests.post(`${serverURL}${apiRoute}/${slug}/${id}/duplicate${locale?.code ? `?locale=${locale.code}` : ""}`, {
      body: JSON.stringify({}),
      headers: {
  ...
  }, [locale, serverURL, apiRoute, slug, id, i18n, t, singularLabel, onDuplicate, redirectAfterDuplicate, setModified, router, adminRoute, collectionConfig, startRouteTransition]);
