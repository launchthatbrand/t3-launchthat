...
export const useField = options => {
  ...
  const result = useMemo(() => ({
    ...
    valid: field?.valid,
    value
  }), [field, processing, setValue, showError, submitted, value, initialValue, path, filterOptions, initializing]);
  ...
  useThrottledEffect(() => {
    ...
    void validateField();
  }, 150, [value, disableFormData, dispatchField, getData, getSiblingData, getDataByPath, id, operation, path, user, validate, field?.rows, collectionSlug]);
  return result;
};
