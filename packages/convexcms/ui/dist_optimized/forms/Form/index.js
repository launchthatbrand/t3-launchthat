...
import { deepCopyObjectSimpleWithoutReactComponents, getDataByPath as getDataByPathFunc, getSiblingData as getSiblingDataFunc, reduceFieldsToValues, wait } from "@convexcms/core/shared";
  ...
  */
  ...
  const submit = useCallback(async (options = {}, e) => {
    ...
    if (Array.isArray(beforeSubmit)) {
      ...
      const isValid = Object.entries(revalidatedFormState).every(([, field]) => field.valid !== false);
      setIsValid(isValid);
      if (!isValid) {
        setProcessing(false);
        setSubmitted(true);
  ...
  const createFormData = useCallback(async (overrides, {
  ...
  const addFieldRow = useCallback(({
    ...
    const newRows = getDataByPath(path) || [];
  ...
  const replaceFieldRow = useCallback(({
    ...
    const currentRows = getDataByPath(path);
  ...
  const executeOnChange = useEffectEvent(submitted => {
    queueTask(async () => {
