...
const RelationshipFieldComponent = props => {
  ...
  const handleValueChange = useEffectEvent(value => {
    const relationMap = createRelationMap({
      hasMany,
      relationTo,
      value
  ...
  const onSave = useCallback(args => {
    ...
    if (hasMany) {
      const unchanged = currentValue.some(option => typeof option === "string" ? option === docID : option.value === docID);
      const valuesToSet = currentValue.map(option => option.value === docID ? {
  ...
  const onDelete = useCallback(args => {
    dispatchOptions({
      id: args.id,
      type: "REMOVE",
      collection: args.collectionConfig,
    ...
    if (hasMany) {
      setValue(valueRef.current ? valueRef.current.filter(option => {
