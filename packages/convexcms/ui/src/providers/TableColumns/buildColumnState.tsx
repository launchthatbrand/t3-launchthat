...
export const buildColumnState = (args: Args): Column[] => {
  ...
  const sortFieldMap = (fieldMap, sortTo) =>
    fieldMap?.sort((a, b) => {
      const aIndex = sortTo.findIndex(
        (column) => "name" in a && column.accessor === a.name,
      );
      const bIndex = sortTo.findIndex(
        (column) => "name" in b && column.accessor === b.name,
      );
  ...
  const sorted: Column[] = sortedFieldMap?.reduce((acc, field, index) => {
    ...
    const columnPreference = columnPreferences?.find(
      (preference) =>
        field && "name" in field && preference.accessor === field.name,
    );
    ...
    const customLabelServerProps: Pick<
      ServerComponentProps,
      "clientField" | "collectionSlug" | "field" | "i18n" | "@convexcms/core"
    > = {
      clientField: field,
    ...
    const Heading = (
      <SortColumn
        ...
        Label={CustomLabel}
        label={
          field && "label" in field ? (field.label as StaticLabel) : undefined
        }
