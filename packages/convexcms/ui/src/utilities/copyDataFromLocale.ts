            ...
            toLocaleData[field.name].map((item: Data, index: number) => {
              if (fromLocaleData[field.name]?.[index]) {
                ...
                if (fieldShouldBeLocalized({ field, parentIsLocalized })) {
                  toLocaleData[field.name][index].id =
            ...
            toLocaleData[field.name].map((blockData: Data, index: number) => {
              const block =
                ...
                ((field.blockReferences ?? field.blocks).find(
                  (block) =>
                    typeof block !== "string" &&
                    block.slug === blockData.blockType,
      ...
      switch (field.type) {
        ...
        case "tabs":
          field.tabs.map((tab) => {
            if (tabHasName(tab)) {
              if (
                tab.name in toLocaleData &&
                fromLocaleData?.[tab.name] !== undefined
              ) {
                iterateFields(
                  tab.fields,
...
export const copyDataFromLocaleHandler = async (
  args: CopyDataFromLocaleArgs,
) => {
  const { req } = args;
...
export const copyDataFromLocale = async (args: CopyDataFromLocaleArgs) => {
  ...
  if (incomingUserSlug) {
    const adminAccessFunction =
      payload.collections[incomingUserSlug].config.access?.admin;
