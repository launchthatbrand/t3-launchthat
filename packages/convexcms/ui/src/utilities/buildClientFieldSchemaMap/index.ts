...
export const buildClientFieldSchemaMap = (args: {
  ...
  if (collectionSlug) {
    ...
    if (matchedCollection) {
      ...
      if (
        matchedCollection.auth &&
        !matchedCollection.auth.disableLocalStrategy
      ) {
        (baseAuthFields[0] as TextFieldClient).label =
          i18n.t("general:password");
        (baseAuthFields[1] as TextFieldClient).label = i18n.t(
    ...
    const matchedGlobal = config.globals.find(
      (global) => global.slug === globalSlug,
    );
