...
import { createClientFields } from "@convexcms/core";
    ...
    switch (field.type) {
          ...
          const block =
            typeof _block === "string"
              ? config.blocksMap
                ...
                : config.blocks.find(
                    (block) =>
                      typeof block !== "string" && block.slug === _block,
                  )
              : _block;
      ...
      case "richText": {
        ...
        for (const [path, subField] of richTextFieldSchemaMap.entries()) {
          ...
          const isFieldsOnly =
            Object.keys(subField).length === 1 && "fields" in subField;
      ...
      case "tabs":
        field.tabs.map((tab, tabIndex) => {
          ...
          const { indexPath: tabIndexPath, schemaPath: tabSchemaPath } =
            getFieldPaths({
              field: {
                ...tab,
                type: "tab",
