...
const DateTimeFieldComponent: DateFieldClientComponent = (props) => {
   ...
   */
  const isDateOnly = ["dayOnly", "default", "monthOnly"].includes(
    pickerAppearance,
  );
      ...
      if (!(readOnly || disabled)) {
        if (timezone && selectedTimezone && incomingDate) {
          ...
          if (isDateOnly) {
            ...
            const dateToSelectedTz = transpose(
              incomingDate,
              TZDateWithSelectedTz,
            );
            ...
            // Transpose the date to the selected timezone
            const dateWithTimezone = transpose(
              dateToUserTz,
              TZDateWithSelectedTz,
            );
  ...
  return (
    ...
    >
      <RenderCustomComponent
        ...
        Fallback={
          <FieldLabel label={label} localized={localized} required={required} />
        }
      />
      <div
