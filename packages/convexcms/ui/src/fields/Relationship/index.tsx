...
const RelationshipFieldComponent: RelationshipFieldClientComponent = (
  props,
) => {
  const {
    field,
      ...
      if (!errorLoading) {
        await relationsToFetch.reduce(async (priorRelation, relation) => {
          ...
          if (resultsFetched < 10) {
            ...
            if (
              relationFilterOption &&
              typeof relationFilterOption !== "boolean"
            ) {
  ...
  const handleValueChange = useEffectEvent((value: Value | Value[]) => {
    ...
    void Object.entries(relationMap).reduce(
    ...
    while (labelString.length > breakApartThreshold) {
      ...
      if (
        r.test(
          labelString.slice(
            0,
            indexOfSpace === -1 ? searchFilter.length : indexOfSpace + 1,
          ),
        )
      ) {
        return true;
      }
      labelString = labelString.slice(
        indexOfSpace === -1 ? searchFilter.length : indexOfSpace + 1,
      );
    }
  ...
  return (
    ...
    >
      ...
      {currentlyOpenRelationship.collectionSlug &&
        currentlyOpenRelationship.hasReadPermission && (
          <DocumentDrawer
            onDelete={onDelete}
