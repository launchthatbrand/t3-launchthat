  ...
  switch (action.type) {
    case "ADD": {
      ...
      const newSubOptions = docs.reduce((docSubOptions, doc) => {
        if (
          loadedIDs.filter(
            (item) => item.id === doc.id && item.relationTo === relation,
          ).length === 0
        ) {
      ...
      ids.forEach((id) => {
        if (
          loadedIDs.filter(
            (item) => item.id === id && item.relationTo === relation,
          ).length === 0
        ) {
    ...
    case "REMOVE": {
      ...
      newOptions[indexOfGroup] = {
        ...newOptions[indexOfGroup],
        options: newOptions[indexOfGroup].options.filter(
    ...
    case "UPDATE": {
      ...
      const foundOption = foundOptionGroup?.options?.find(
        (option) => option.value === doc.id,
      );
