...
export const EditUpload: React.FC<EditUploadProps> = ({
  ...
  const fineTuneCrop = ({
    ...
    const percentage =
      100 *
      (intValue /
        (dimension === "width" ? uncroppedPixelWidth : uncroppedPixelHeight));
    ...
    if (typeof onSave === "function") {
      onSave({
        ...
        heightInPixels: Number(
          heightInputRef?.current?.value ?? uncroppedPixelHeight,
        ),
        widthInPixels: Number(
          widthInputRef?.current?.value ?? uncroppedPixelWidth,
        ),
      });
    }
  ...
  const fileSrcToUse = imageCacheTag
    ? `${fileSrc}?${encodeURIComponent(imageCacheTag)}`
    : fileSrc;
              ...
              <div className={`${baseClass}__groupWrap`}>
                ...
                <div className={`${baseClass}__inputsWrap`}>
                  <Input
                    ...
                    onChange={(value) =>
                      fineTuneCrop({ dimension: "width", value })
                    }
                    ref={widthInputRef}
                    value={((crop.width / 100) * uncroppedPixelWidth).toFixed(
                      0,
                    )}
                  />
                  <Input
                    ...
                    onChange={(value) =>
                      fineTuneCrop({ dimension: "height", value })
                    }
                    ref={heightInputRef}
                    value={((crop.height / 100) * uncroppedPixelHeight).toFixed(
                      0,
                    )}
                  />
                </div>
...
const DraggableElement = ({
  ...
  const getCoordinates = React.useCallback(
    (mouseXArg?: number, mouseYArg?: number, recenter?: boolean) => {
      ...
      const xOutOfBounds =
        mouseX < boundsRect.left || mouseX > boundsRect.right;
      const yOutOfBounds =
        mouseY < boundsRect.top || mouseY > boundsRect.bottom;
      ...
      if (xOutOfBounds || yOutOfBounds) {
        ...
        if (mouseX < boundsRect.left) {
          x =
            ((boundsRect.left - containerRect.left) / containerRect.width) *
            100;
        ...
        if (mouseY < boundsRect.top) {
          y =
            ((boundsRect.top - containerRect.top) / containerRect.height) * 100;
        } else if (mouseY > boundsRect.bottom) {
          y =
            ((containerRect.height -
              (containerRect.bottom - boundsRect.bottom)) /
              containerRect.height) *
            100;
        }
  ...
  return (
    ...
    >
      <button
        className={[`${baseClass}__draggable`, className]
          .filter(Boolean)
          .join(" ")}
