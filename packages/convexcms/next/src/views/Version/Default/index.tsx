...
export const DefaultVersionView: React.FC<DefaultVersionsViewProps> = ({
  ...
  const [selectedLocales, setSelectedLocales] =
    useState<OptionObject[]>(selectedLocalesProp);
  ...
  useEffect(() => {
    ...
    if (!selectedLocales) {
      ...
      current.set(
        "localeCodes",
        JSON.stringify(selectedLocales.map((locale) => locale.value)),
      );
    }
  ...
  const draftsEnabled = Boolean(
    (collectionConfig || globalConfig)?.versions.drafts,
  );
        ...
        <div className={`${baseClass}__header-wrap`}>
          <p className={`${baseClass}__created-at`}>
            {i18n.t("version:versionCreatedOn", {
              version: i18n.t(
                doc?.autosave ? "version:autosavedVersion" : "version:version",
              ),
            })}
          </p>
