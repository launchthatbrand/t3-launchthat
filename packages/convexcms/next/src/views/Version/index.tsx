...
import { getClientSchemaMap } from "@convexcms/ui/utilities/getClientSchemaMap";
import { getSchemaMap } from "@convexcms/ui/utilities/getSchemaMap";
...
export async function VersionView(props: DocumentViewServerProps) {
  ...
  const comparisonVersionIDFromParams: string =
    searchParams.compareValue as string;

  const modifiedOnly: boolean =
    searchParams.modifiedOnly === "false" ? false : true;
  ...
  if (localization) {
    ...
    if (localization.filterAvailableLocales) {
      locales =
        (await localization.filterAvailableLocales({ locales, req })) || [];
    }
  ...
  const RenderedDiff = RenderDiff({
    ...
    req,
    selectedLocales:
      selectedLocales && selectedLocales.map((locale) => locale.value),
