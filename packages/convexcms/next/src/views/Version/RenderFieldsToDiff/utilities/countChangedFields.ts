...
import { fieldShouldBeLocalized } from "@convexcms/core/shared";
  ...
  fields.forEach((field) => {
    ...
    switch (fieldType) {
      ...
      case "upload": {
        ...
        if (locales && fieldShouldBeLocalized({ field, parentIsLocalized })) {
          locales.forEach((locale) => {
            if (
              fieldHasChanges(
                version?.[field.name]?.[locale],
                comparison?.[field.name]?.[locale],
              )
            ) {
              count++;
            }
          ...
          fieldHasChanges(version?.[field.name], comparison?.[field.name])
        ) {
          count++;
        }
        break;
      ...
      default: {
        ...
        throw new Error(
          `Unexpected field.type in countChangedFields : ${String(fieldType)}`,
        );
      }
    }
  });
