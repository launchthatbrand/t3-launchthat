...
export type BuildVersionFieldsArgs = {
  ...
  customDiffComponents: Partial<
    Record<
      FieldTypes,
      PayloadComponent<FieldDiffServerProps, FieldDiffClientProps>
    >
  >;
  entitySlug: string;
  fieldPermissions:
...
export const buildVersionFields = ({
  ...
  for (const field of fields) {
    ...
    if (!clientField) {
      ...
      throw new Error(
        "No client field found for " + entitySlug + "." + schemaPath,
      );
    }
    ...
    if (isLocalized) {
      ...
      const baseVersionField = buildVersionField({
        ...
        modifiedOnly,
        parentIsLocalized:
          parentIsLocalized || ("localized" in field && field.localized),
        parentPath,
        parentSchemaPath,
        path,
        req,
    ...
    for (const tab of field.tabs) {
      ...
      baseVersionField.tabs.push({
        ...
        fields: buildVersionFields({
          clientSchemaMap,
          comparisonSiblingData:
            "name" in tab ? comparisonValue?.[tab.name] : comparisonValue,
          customDiffComponents,
          entitySlug,
          fieldPermissions,
          fields: tab.fields,
          ...
          selectedLocales,
          versionSiblingData:
            "name" in tab ? versionValue?.[tab.name] : versionValue,
        }).versionFields,
        label: tab.label,
      });
    }
    ...
    for (let i = 0; i < blocksValue.length; i++) {
      ...
      const versionBlock =
        ...
        ((field.blockReferences ?? field.blocks).find(
          (block) =>
            typeof block !== "string" && block.slug === blockSlugToMatch,
      ...
      if (versionRow.blockType === comparisonRow.blockType) {
        ...
        const comparisonBlock =
          ...
          ((field.blockReferences ?? field.blocks).find(
            (block) =>
              typeof block !== "string" &&
              block.slug === comparisonBlockSlugToMatch,
          ) as FlattenedBlock | undefined);
