...
type OneSegmentViews = {
  [K in Exclude<
    keyof SanitizedConfig["admin"]["routes"],
    "reset"
...
export const getViewFromConfig = ({
  ...
  const [segmentOne, segmentTwo, segmentThree, segmentFour, segmentFive] =
    segments;
  ...
  switch (segments.length) {
    ...
    case 1: {
      ...
      if (config.admin.routes) {
        const matchedRoute = Object.entries(config.admin.routes).find(
          ([, route]) => {
