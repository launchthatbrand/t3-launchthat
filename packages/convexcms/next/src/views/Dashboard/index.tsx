...
import type { EntityToGroup } from "@convexcms/ui/shared";
import React, { Fragment } from "react";
import { HydrateAuthProvider, SetStepNav } from "@convexcms/ui";
import { RenderServerComponent } from "@convexcms/ui/elements/RenderServerComponent";
import { EntityType, groupNavItems } from "@convexcms/ui/shared";
...
export async function Dashboard({
  ...
  if (config.globals.length > 0) {
    ...
    globalData = config.globals.map((global) => {
      ...
      const lockedDoc = lockedDocuments.docs.find(
        (doc) => doc.globalSlug === global.slug,
      );
