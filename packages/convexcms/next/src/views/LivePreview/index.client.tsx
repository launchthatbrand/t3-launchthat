...
const PreviewView: React.FC<Props> = ({
  ...
  const lockDocumentsProp =
    docConfig?.lockDocuments !== undefined ? docConfig?.lockDocuments : true;
  ...
  const [isReadOnlyForIncomingUser, setIsReadOnlyForIncomingUser] =
    useState(false);
  ...
  const onChange: FormProps["onChange"][0] = useCallback(
    async ({ formState: prevFormState, submitted }) => {
      ...
      if (isLockingEnabled) {
        ...
        if (lockedState) {
          const lockedUserID =
            typeof lockedState.user === "string" ||
            typeof lockedState.user === "number"
              ? lockedState.user
              : lockedState.user.id;

          if (
            !documentLockStateRef.current ||
            lockedUserID !== previousOwnerID
          ) {
            ...
            documentLockStateRef.current = documentLockStateRef.current = {
              hasShownLockedModal:
                documentLockStateRef.current?.hasShownLockedModal || false,
              isLocked: true,
      ...
      >
        {isLockingEnabled &&
          ...
          !isReadOnlyForIncomingUser && (
            <DocumentLocked
              handleGoBack={() =>
                handleGoBack({ adminRoute, collectionSlug, router })
              }
        ...
        {isLockingEnabled && showTakeOverModal && (
          <DocumentTakeOver
            handleBackToDashboard={() =>
              handleBackToDashboard({ adminRoute, router })
            }
        ...
        <SetDocumentStepNav
          ...
          id={id}
          pluralLabel={
            collectionConfig ? collectionConfig?.labels?.plural : undefined
          }
          useAsTitle={
            collectionConfig ? collectionConfig?.admin?.useAsTitle : undefined
          }
