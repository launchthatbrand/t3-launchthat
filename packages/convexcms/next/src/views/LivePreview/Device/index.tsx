...
import { useResize } from "@convexcms/ui";
...
export const DeviceContainer: React.FC<{
  ...
  if (breakpoint && breakpoint !== "responsive") {
    ...
    if (
      ...
      const scaledDeviceDifferencePixels =
        scaledDesiredWidth - desiredSize.width;
      deviceIsLargerThanFrame = scaledDeviceWidth > outerFrameSize.width;

      if (deviceIsLargerThanFrame) {
        if (zoom > 1) {
          const differenceFromDeviceToFrame =
            measuredDeviceSize.width - outerFrameSize.width;
        ...
        if (zoom >= 1) {
          ...
          const differenceFromDeviceToFrame =
            outerFrameSize.width - scaledDeviceWidth;
          x = `${differenceFromDeviceToFrame / 2}px`;
          margin = "0";
        }
      }
