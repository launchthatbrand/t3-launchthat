...
import { logError } from "@convexcms/core";
...
import { isEditing as getIsEditing } from "@convexcms/ui/shared";
import { buildFormState } from "@convexcms/ui/utilities/buildFormState";
...
export const generateMetadata: GenerateEditViewMetadata = async (args) =>
...
export const renderDocument = async ({
  ...
  const [
    ...
    buildFormState({
      ...
      locale: locale?.code,
      operation:
        (collectionSlug && idFromArgs) || globalSlug ? "update" : "create",
  ...
  if (collectionConfig) {
    if (
      !visibleEntities?.collections?.find(
        (visibleSlug) => visibleSlug === collectionSlug,
      ) &&
      !overrideEntityVisibility
    ) {
      throw new Error("not-found");
  ...
  if (globalConfig) {
    if (
      !visibleEntities?.globals?.find(
        (visibleSlug) => visibleSlug === globalSlug,
      )
    ) {
      throw new Error("not-found");
    }
