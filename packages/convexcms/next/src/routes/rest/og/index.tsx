...
import type { PayloadHandler } from "@convexcms/core";
import React from "react";
import { ImageResponse } from "next/og.js";
import { PayloadIcon } from "@convexcms/ui/shared";
...
export const generateOGImage: PayloadHandler = async (req) => {
  ...
  try {
    ...
    const leader = hasLeader
      ? searchParams.get("leader")?.slice(0, 100).replace("-", " ")
      : "";
    ...
    try {
      ...
      req.payload.logger.error(
        `Error reading font file or not readable: ${e.message}`,
      );
    }
