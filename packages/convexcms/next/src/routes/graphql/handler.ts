...
const handleError = async ({
  ...
  const status =
    (err.originalError as APIError).status || httpStatus.INTERNAL_SERVER_ERROR;
  ...
  let response: GraphQLFormattedError = {
    extensions: {
      ...
      data:
        (err && err.originalError && (err.originalError as APIError).data) ||
        undefined,
...
export const POST =
  ...
  async (request: Request) => {
    ...
    const apiResponse = await createHandler({
      ...
      schema,
      validationRules: (_, args, defaultRules) =>
