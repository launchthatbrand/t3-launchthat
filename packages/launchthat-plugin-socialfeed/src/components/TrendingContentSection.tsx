...
export function TrendingContentSection({
  ...
  const popularTopics = useSocialFeedQuery(
    socialfeedApi?.plugins?.socialfeed?.queries?.getTopics,
    {
  ...
  const trendingContent = useSocialFeedQuery(
    socialfeedApi?.plugins?.socialfeed?.queries?.getTrendingContent,
    {
      ...
      <CardContent>
        ...
        >
          <TabsList className="mb-4 w-full max-w-full overflow-x-auto">
            ...
            {!isLoading &&
              popularTopics?.map(
                (topic: { _id: Id<"hashtags">; tag: string }) => (
                  <TabsTrigger
            ...
            {isLoading ? (
              <div className="flex items-center justify-center py-8">
                <div className="text-center">
                  <TrendingUp className="text-muted-foreground mx-auto h-10 w-10 animate-pulse" />
              ...
              <div className="flex items-center justify-center py-8">
                <div className="text-center">
                  <TrendingUp className="text-muted-foreground mx-auto h-10 w-10" />
                  <p className="text-muted-foreground mt-2 text-sm">
