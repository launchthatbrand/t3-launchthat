...
export function HashtagSuggestion({
  ...
  const topics = useSocialFeedQuery(
  ...
  const suggestions = useMemo<HashtagSuggestionType[] | undefined>(() => {
    if (!Array.isArray(topics)) return undefined;
    return topics.map((topic) => ({
      _id: topic._id,
      tag: topic.tag,
      usageCount: (topic as { usageCount?: number }).usageCount ?? 0,
    }));
  }, [topics]);
  ...
  return (
    <div
      className="border-border bg-popover absolute z-50 w-64 overflow-hidden rounded-md border shadow-md"
      style={style}
    >
      <Command>
