...
export function PostCreator({
  ...
  const { userId: clerkUserId } = useSocialFeedAuth();

  // Convert Clerk user ID to Convex user ID
  const convexUser = useSocialFeedQuery(
    socialfeedApi?.core?.users?.queries?.getUserByClerkId,
  ...
  const groups = (useSocialFeedQuery(
    socialfeedApi?.groups?.queries?.getUserGroups,
  ...
  // Mutations
  const createPost = useSocialFeedMutation(socialfeedApi.mutations?.createPost);
  const updatePost = useSocialFeedMutation(socialfeedApi.mutations?.updatePost);

  // If editing, fetch the existing post - using any temporarily until proper types are available
  const existingPost = useSocialFeedQuery(
              ...
              <div className="mt-4 space-y-2">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {mediaItems.map((media, index) => (
                    <div
                      key={index}
                      className="bg-muted relative aspect-video rounded-md"
                    >
                      {media.type === "image" ? (
                        <img
                          src={media.url}
