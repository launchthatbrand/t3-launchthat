...
// import type { Doc } from "../../_generated/dataModel"; // REMOVED - unused (Id is used)
 ...
 */
export const getAuthenticatedUserDocIdByToken = async (
...
const levelsOrder: PermissionLevel[] = ["none", "own", "group", "all"];
...
export const hasPermission = async (
  ...
  if (userRoles.length === 0) {
    ...
    if (
      ...
      return false;
    if (directPermission.level === "none") return false;
  ...
  for (const roleId of roleIds) {
    ...
    if (!("name" in role)) {
      console.warn(
        `Role with ID ${roleId} does not have a name property. Skipping for permission check.`,
      );
      continue;
    }
    ...
    const rolePermission = await ctx.db
      ...
      .withIndex("by_role_permission", (q) =>
        q.eq("roleId", roleId).eq("permissionKey", permissionKey),
      )
  ...
  if (highestLevel === "none") return false;
  ...
  if (highestLevel === "group" && scopeType === "global") return false;
...
export const requirePermission = async (
  ...
  if (!hasPerm) {
    throw new Error(
      `Permission denied: User ${userId} does not have permission '${permissionKey}'` +
        (scopeType !== "global"
