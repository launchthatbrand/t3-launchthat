...
export const create = mutation({
  args: {
    ...
    type: v.string(),
    label: v.string(),
    nodeId: v.string(), // New: integration node identifier
    connectionId: v.optional(v.id("connections")), // New: optional connection reference
    config: v.any(), // Changed from v.string() to v.any() for flexible JSON
    position: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("nodes", {
      ...
      type: args.type,
      label: args.label,
      nodeId: args.nodeId, // New field
      connectionId: args.connectionId, // New optional field
      config: args.config, // Now flexible JSON
...
interface NodeUpdate {
  label?: string;
  config?: unknown; // Changed from any to unknown for better type safety
  position?: string;
  order?: number;
  connectionId?: Id<"connections">; // Fixed: use proper type
  updatedAt: number;
}
...
export const update = mutation({
  args: {
    ...
    config: v.optional(v.any()), // Changed from v.string() to v.any()
    position: v.optional(v.string()),
    order: v.optional(v.number()),
    connectionId: v.optional(v.id("connections")), // Fixed: proper validator syntax
  },
  returns: v.id("nodes"),
  handler: async (ctx, args) => {
    ...
    if (updates.config !== undefined)
      updateData.config = updates.config as unknown;
    if (updates.position !== undefined) updateData.position = updates.position;
    if (updates.order !== undefined) updateData.order = updates.order;
    if (updates.connectionId !== undefined)
      updateData.connectionId = updates.connectionId;
