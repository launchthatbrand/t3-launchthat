...
export function CommentThread({
  ...
  const updateComment = useMutation(api.socialfeed.mutations.updateComment);
  const deleteComment = useMutation(api.socialfeed.mutations.deleteComment);
  ...
  const handleLoadMore = async () => {
    ...
    if (comments.length > 0) {
      setCursor(comments[comments.length - 1]._id);
    } else {
  ...
  const getReplies = (parentCommentId: Id<"comments">) => {
    ...
    const replies =
      useQuery(api.socialfeed.queries.getCommentReplies, {
        parentCommentId,
        feedItemId: postId,
          ...
          <div className="flex-1">
            ...
            <div className="mt-1 flex items-center gap-3 text-xs text-muted-foreground">
              ...
              {comment.repliesCount > 0 && (
                <Button
                  variant="link"
                  className="flex h-auto items-center p-0 text-xs text-muted-foreground"
            ...
            {isExpanded && replies.length > 0 && (
              <div className="mt-3 space-y-4 border-l-2 border-muted pl-3">
      ...
      <form onSubmit={handleSubmitComment}>
        <div className="flex gap-2">
          ...
          <div className="flex-1">
            ...
            {/* Mention dropdown */}
            {showMentionDropdown && (
              <div className="absolute z-10 mt-1 w-64 rounded-md border bg-background shadow-lg">
