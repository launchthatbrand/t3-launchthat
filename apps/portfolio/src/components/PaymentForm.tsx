...
const PaymentForm: React.FC<PaymentFormProps> = ({ apiLoginId, clientKey }) => {
  ...
  const onSuccessHandler = async (response: AuthorizeNetClientResponse) => {
    ...
    try {
      const convexHttpUrl = env.NEXT_PUBLIC_CONVEX_URL;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {/* Credit Card Option */}
        <div
          className={`rounded-md border p-4 ${paymentMethod === "credit-card" ? "border-primary ring-1 ring-primary" : "border-input"}`}
        >
          <label
            htmlFor="credit-card"
        ...
        <div
          className={`rounded-md border p-4 ${paymentMethod === "sezzle" ? "border-primary ring-1 ring-primary" : "border-input"}`}
        >
          <label
            htmlFor="sezzle"
            className="flex cursor-pointer items-center justify-between"
        ...
        <div
          className={`rounded-md border p-4 ${paymentMethod === "crypto" ? "border-primary ring-1 ring-primary" : "border-input"}`}
        >
          <label
            htmlFor="crypto"
            className="flex cursor-pointer items-center justify-between"
