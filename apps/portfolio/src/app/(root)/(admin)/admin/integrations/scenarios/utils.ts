...
// ScenarioId is imported from types.ts
...
/**
 * Parse app configuration template to extract triggers and actions
 * @param configTemplate - The JSON string from the app's configTemplate field
 * @returns Array of triggers and actions for the app
 */
export function parseAppActions(configTemplate: string): ActionDefinition[] {
  try {
    ...
    if (
      ...
      Object.entries(config.triggers.default).forEach(
        ([key, trigger]: [string, any]) => {
          if (trigger && typeof trigger === "object") {
            actions.push({
              id: trigger.id || key.toLowerCase().replace(/\s+/g, "_"),
              name: key,
              type: "trigger",
      ...
      Object.entries(config.actions.default).forEach(
        ([key, action]: [string, any]) => {
          if (action && typeof action === "object") {
            actions.push({
              id: action.id || key.toLowerCase().replace(/\s+/g, "_"),
              name: key,
              type: "action",
              description: action.description || `${key} action`,
            });
          }
        },
      );
    ...
    console.error("Failed to parse app configuration template:", error);
    return [];
  }
}
 ...
 * @param nodeType - Whether to filter for triggers or actions
...
export function getAppActions(
  ...
  if (!app.configTemplate) {
    return [];
  }

  const allActions = parseAppActions(app.configTemplate);

  if (nodeType === "trigger") {
    return allActions.filter((action) => action.type === "trigger");
  }

  return allActions.filter((action) => action.type === "action");
}
