...
export async function GET(req: NextRequest) {
  ...
  if (env.NODE_ENV !== "production") {
    ...
    const isLocalIssuer = issuer.includes("localhost") || issuer.includes("127.0.0.1");
  ...
  // Keep the Convex client typed loosely to avoid TS instantiation-depth issues.
  ...
  const session: unknown = await convexAny.query(
    // This is a thin public query we add to the TraderLaunchpad Convex app.
