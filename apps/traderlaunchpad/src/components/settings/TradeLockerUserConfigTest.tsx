...
export function TradeLockerUserConfigTest() {
  const probe = useAction(api.traderlaunchpad.actions.probeMyTradeLockerConfig);
  ...
  return (
    <Card className="border-white/10 bg-white/3 backdrop-blur-md">
      <CardHeader className="pb-3">
        <CardTitle className="text-base">/trade/config (user token)</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="text-sm text-white/60">
          Fetches <span className="font-mono">/trade/config</span> using your connected TradeLocker JWT +{" "}
          <span className="font-mono">accNum</span> header.
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            ...
            disabled={loading}
          >
            {loading ? "Fetching..." : "Fetch TradeLocker config"}
          </Button>
        </div>
