...
export const EconomicCalendarEmbedMeDialog = ({
  newsBase,
  widgetStyle,
  defaultApiBase = "https://different-trout-684.convex.site",
}: Props) => {
  ...
  const embedSnippetReact = React.useMemo(() => {
    const currenciesLine = embedConfig.hasCurrencies ? `        currencies="${embedConfig.currenciesCsv}"` : "";
    return [
      `import * as React from "react";`,
      ``,
      ...
      .filter(Boolean)
      .join("\n");
  }, [embedAccent, embedBg, embedBorder, embedConfig, embedFg, embedMuted, embedRadius]);
