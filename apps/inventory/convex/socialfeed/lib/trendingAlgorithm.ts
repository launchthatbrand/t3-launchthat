...
export function calculateTrendingScore(metrics: {
  ...
  comments: number;
  shares: number;
  creationTime: number; // Content creation timestamp
...
export async function updateTrendingMetrics(
  ...
  if (existingTrending) {
    ...
    if (timeWindowMs > 0) {
      reactionVelocity = calculateVelocity(
        reactions,
        existingTrending.reactions,
        timeWindowMs,
      );

      commentVelocity = calculateVelocity(
        comments,
        existingTrending.comments,
        timeWindowMs,
      );

      shareVelocity = calculateVelocity(
        shares,
        existingTrending.shares,
        timeWindowMs,
      );
    }
  }
 ...
 */
export function calculateContentQuality(content: Doc<"feedItems">): number {
  let score = 1.0;
  ...
  if (content.mediaUrls && content.mediaUrls.length > 0) {
    score += 0.5;
    if (content.mediaUrls.length > 1) score += 0.2;
  }

  // Original posts vs shares (original content might be more valuable)
  if (content.contentType === "post") score += 0.2;
