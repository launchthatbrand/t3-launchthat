...
export const updateTrendingAndRecommendations = internalAction({
  ...
  handler: async (ctx) => {
    ...
    await ctx.runMutation(
      internal.socialfeed.crons.generateRecommendationsForActiveUsers,
      {},
    );
  ...
  handler: async (ctx) => {
    ...
    for (const userId of activeUserIds) {
      try {
        ...
        console.error(
          `Error generating recommendations for user ${userId.toString()}:`,
          error,
        );
      }
    }
...
crons.interval(
  ...
  { hours: 6 },
  internal.socialfeed.crons.updateTrendingAndRecommendations,
  {},
);
