 ...
 * Progress Helpers
 *
 * Contains shared utility functions for the progress feature.
 */
 ...
 */
export const canAccessNextItem = async (
  ctx: QueryCtx,
  userId: Id<"users">,
  courseId: Id<"courses">,
  currentItemId: Id<"lessons"> | Id<"topics"> | Id<"quizzes">,
): Promise<boolean> => {
  ...
  for (const topic of topics) {
    const progress = await ctx.db
      ...
      .withIndex("by_user_item", (q) =>
        q.eq("userId", userId).eq("itemId", topic._id),
      )
      .unique();
 ...
 */
export const getNextRecommendedItem = async (
  ctx: QueryCtx,
  userId: Id<"users">,
  courseId: Id<"courses">,
): Promise<{
