      ...
      return {
        ...
        config:
          typeof nrec.config === "string"
            ? (nrec.config as string)
            : JSON.stringify((nrec.config as Record<string, unknown>) ?? {}),
        position:
          typeof nrec.position === "string"
            ? (nrec.position as string)
            : JSON.stringify(
                (nrec.position as Record<string, unknown>) ?? {
                  x: 0,
                  y: 0,
                },
              ),
        createdAt:
          typeof nrec.createdAt === "number"
            ? (nrec.createdAt as number)
            : n._creationTime,
        updatedAt:
          typeof nrec.updatedAt === "number"
            ? (nrec.updatedAt as number)
            : n._creationTime,
      };
    });
      ...
      return {
        ...
        mapping:
          typeof crec.mapping === "string" || crec.mapping === undefined
            ? (crec.mapping as string | undefined)
            : JSON.stringify(crec.mapping),
        createdAt:
          typeof crec.createdAt === "number"
            ? (crec.createdAt as number)
            : c._creationTime,
        updatedAt:
          typeof crec.updatedAt === "number"
            ? (crec.updatedAt as number)
            : c._creationTime,
      };
    });
