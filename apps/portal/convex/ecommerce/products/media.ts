...
export const removeProductMedia = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const remainingMediaItems = (product.mediaItems || []).filter(
      (item) => !args.mediaItemIds.includes(item.mediaItemId),
    );

    // Regenerate images from remaining media items
    const newImages =
      remainingMediaItems.length > 0
    ...
    return {
      success: true,
      removedCount:
        (product.mediaItems?.length || 0) - remainingMediaItems.length,
    };
  },
});
...
export const getProductWithMedia = query({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      mediaItems:
        mediaItemsWithUrls.length > 0 ? mediaItemsWithUrls : undefined,
    };
  },
});
...
export const createProductWithMedia = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const mediaItemIds: Id<"mediaItems">[] = [];
    ...
    if (args.storageIds && args.storageIds.length > 0) {
      for (let i = 0; i < args.storageIds.length; i++) {
        ...
        const mediaResult = await ctx.runMutation(
          api.media.createMediaForPost,
          {
            storageId,
