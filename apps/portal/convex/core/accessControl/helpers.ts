...
import type { DownloadPermission, DownloadRole } from "./types";
...
export const hasDownloadPermission = async (
  ...
  const userDownloadRole = (user as any).downloadRole as
    | DownloadRole
    | undefined;
  const effectiveRole =
...
export const checkDownloadAccess = async (
  ...
  if (download.requiredProductId) {
    ...
    const hasRequiredProduct = userOrders.some((order) =>
      order.items.some((item) => item.productId === download.requiredProductId),
    );

    if (hasRequiredProduct) return true;
  }
