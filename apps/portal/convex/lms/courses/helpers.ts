 ...
 * Courses Helpers
 *
 * Contains shared utility functions for the courses feature.
 */
...
export const isUserEnrolledInCourse = async (
  ...
  const enrollment = await ctx.db
    .query("courseEnrollments")
    .filter((q) =>
      q.eq(q.field("userId"), userId).eq(q.field("courseId"), courseId),
    )
    .first();
...
export const validateCourseStructure = async (
  ...
  isValid: boolean;
  missingLessonIds: Id<"lessons">[];
}> => {
  const course = await ctx.db.get(courseId);
  if (!course || !course.courseStructure) {
    return { isValid: true, missingLessonIds: [] };
  }

  const missingLessonIds: Id<"lessons">[] = [];
