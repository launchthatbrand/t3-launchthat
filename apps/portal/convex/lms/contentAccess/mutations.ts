...
// Validator for tag selector objects
const tagSelectorValidator = v.object({
  mode: v.union(v.literal("all"), v.literal("some")),
  tagIds: v.array(v.id("marketingTags")),
});

const emptySelector: MarketingTag = { mode: "some", tagIds: [] };
...
export const saveContentAccessRules = mutation({
  args: {
    ...
    requiredTags: v.optional(tagSelectorValidator),
    excludedTags: v.optional(tagSelectorValidator),
  },
  ...
  handler: async (ctx, args) => {
    ...
    if (existingRule) {
      ...
      await ctx.db.patch(existingRule._id, {
        ...
        requiredTags: args.requiredTags ?? emptySelector,
        excludedTags: args.excludedTags ?? emptySelector,
        updatedAt: now,
      });
