...
import type { GroupMemberAPIStatus, GroupWithDetails } from "./schema/types";
...
export const listGroups = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.search && args.search.trim() !== "") {
      ...
      if (args.filters.privacy) {
        queryBuilder = queryBuilder.filter((q) =>
          q.eq(q.field("privacy"), args.filters.privacy),
        );
      }
    }
    ...
    const groupsWithDetails = await Promise.all(
      paginatedResults.page.map(async (group) => {
        // Get member count
        ...
        if (userId) {
          ...
          if (membership) {
            ...
            let mappedStatus: GroupMemberAPIStatus;
        ...
        if (
          args.filters.categoryTags &&
          args.filters.categoryTags.length > 0 &&
          (!group.categoryTags ||
            !args.filters.categoryTags.some((tag) =>
              group.categoryTags?.includes(tag),
            ))
        ) {
    ...
    const filteredGroups = groupsWithDetails.filter(
  ...
  handler: async (ctx, args) => {
    ...
    if (identity) {
      ...
      if (user) {
        ...
        console.log(
          "[getGroupById] No user found for token:",
  ...
  handler: async (ctx, args) => {
    ...
    if (args.filter && args.filter !== "all") {
      if (args.filter === "pending") {
        ...
        queryBuilder = queryBuilder.filter((q) =>
          q.eq(q.field("status"), args.filter),
        );
      }
    }
    ...
    const membersWithUserDetails = await Promise.all(
      paginatedResults.page.map(async (member) => {
...
export const getGroupMembers = query({
  args: {
    ...
    roleFilter: v.optional(
      v.union(v.literal("admin"), v.literal("moderator"), v.literal("member")),
    ),
    paginationOpts: v.optional(paginationOptsValidator),
  },
  handler: async (ctx, args) => {
...
export const getUserMembershipInGroup = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!user) {
      console.log(
        "[getUserMembershipInGroup] No user found for token:",
        identity.tokenIdentifier,
      );
      return null;
    }
