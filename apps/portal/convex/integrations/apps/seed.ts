...
export const seedDefaultApps = mutation({
  ...
  handler: async (ctx) => {
    ...
    const webhooksApp = existingApps.find(
      (app) => app.name.toLowerCase() === "webhooks",
    );
    ...
    if (!webhooksApp) {
      await ctx.db.insert("apps", {
        name: "Webhooks",
        description:
          "Send HTTP webhooks to external services when events occur.",
        authType: "none",
        isInternal: true,
        configTemplate: JSON.stringify({
          webhookUrl: {
    ...
    const traderLaunchpadApp = existingApps.find(
      (app) => app.name.toLowerCase() === "traderlaunchpad",
    );
    ...
    if (!traderLaunchpadApp) {
      await ctx.db.insert("apps", {
        ...
        description:
          "Portal integration for TraderLaunchpad with order triggers and actions.",
        authType: "internal",
        isInternal: true,
        configTemplate: JSON.stringify({
          enabled: {
    ...
    const connectionResult = await ctx.runMutation(
    ...
    console.log(
      `Created ${connectionResult.connectionsCreated} internal connections`,
    );
