...
type PostType = Doc<"contentTypes"> & {
  supports?: PostTypeSupports;
  rewrite?: PostTypeRewrite;
  adminMenu?: PostTypeAdminMenu;
};
...
export default function PostTypesSettingsPage() {
  ...
  const handleRefreshCounts = async () => {
    try {
      ...
      toast(`Updated entry counts for ${count} post types`);
    } catch (error) {
          ...
          <div className="mb-4 flex justify-between">
            <div>
              <h2 className="text-xl font-semibold">
                {selectedType
                  ? `Fields for ${mockPostTypes.find((t) => t.id === selectedType)?.name}`
                  : "Fields"}
              </h2>
              {!selectedType && (
                <p className="mt-1 text-sm text-muted-foreground">
                  Select a post type to manage its fields
                </p>
              )}
            </div>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        ...
                        {mockPostTypes.find((t) => t.id === selectedType)?.name}{" "}
                        post type
                      </DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
