...
export default function CategoriesAdminPage() {
  ...
  const categoriesData =
    useQuery(api.ecommerce.categories.index.getProductCategories, {
      // The API accepts isActive and isVisible, not includeInactive/includeInvisible
      // When these are undefined, the API will include all categories
    }) ?? [];

  // Map to add the missing productCount property
  const categoryTree = categoriesData.map((category) => ({
    ...category,
  ...
  // Map to add the missing productCount property
  const allCategories = allCategoriesData.map((category) => ({
    ...category,
    productCount: 0, // Default to 0 since we don't have actual product counts
  }));
