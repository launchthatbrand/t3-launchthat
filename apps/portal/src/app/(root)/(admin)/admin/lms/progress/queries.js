...
export const getCourseProgress = query({
    ...
    returns: v.union(v.null(), v.object({
        ...
        completed: v.number(),
        total: v.number(),
        lastAccessedId: v.optional(v.union(v.id("lessons"), v.id("topics"), v.id("quizzes"))),
        lastAccessedType: v.optional(v.union(v.literal("lesson"), v.literal("topic"), v.literal("quiz"))),
    ...
    handler: async (ctx, args) => {
        ...
        const percentComplete = courseProgress.total > 0
...
export const isItemCompleted = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.courseId) {
            queryBuilder = queryBuilder.filter((q) => q.eq(q.field("courseId"), args.courseId));
        }
...
export const isLessonCompleted = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (topics.length === 0) {
            ...
            return await ctx.db
                .query("progress")
                .withIndex("by_user_item", (q) => q.eq("userId", args.userId).eq("itemId", args.lessonId))
                .unique()
        ...
        for (const topic of topics) {
            const progress = await ctx.db
                .query("progress")
                .withIndex("by_user_item", (q) => q.eq("userId", args.userId).eq("itemId", topic._id))
                .unique();
...
export const getDetailedCourseProgress = query({
    ...
    returns: v.object({
        courseProgress: v.union(v.null(), v.object({
            lessons: v.object({
                completed: v.array(v.id("lessons")),
                total: v.number(),
            }),
    ...
    handler: async (ctx, args) => {
        ...
        const formattedCourseProgress = courseProgress
            ? {
                ...courseProgress,
                percentComplete: courseProgress.total > 0
                    ? Math.round((courseProgress.completed / courseProgress.total) * 100)
                    : 0,
            }
            : null;
    ...
    returns: v.object({
        ...
        lessons: v.array(v.object({
            ...
            progress: v.object({
                ...
                status: v.union(v.literal("not_started"), v.literal("in_progress"), v.literal("completed"), v.literal("locked")),
            }),
    ...
    handler: async (ctx, args) => {
        ...
        for (let i = 0; i < lessons.length; i++) {
            ...
            const completedItems = await ctx.db
                ...
                .filter((q) => q.eq(q.field("completed"), true))
                .collect();
            const completedItemIds = new Set(completedItems.map((item) => item.itemId.toString()));
            // Check lesson completion
            const lessonCompleted = completedItemIds.has(lesson._id.toString());
            // Check topic completion
            const completedTopicsInLesson = topicIds.filter((id) => completedItemIds.has(id.toString()));
            completedTopicsCount += completedTopicsInLesson.length;
            // Check quiz completion
            const completedQuizzesInLesson = quizIds.filter((id) => completedItemIds.has(id.toString()));
            ...
            const allTopicsCompleteForStats = completedTopicsInLesson.length === topics.length;
            const allQuizzesCompleteForStats = completedQuizzesInLesson.length === quizzes.length;
            ...
            if (lessonEffectivelyComplete)
                completedLessonsCount++;
            // Calculate lesson progress percentage
            // A lesson is considered complete when all its topics and quizzes are complete
            const allTopicsComplete = completedTopicsInLesson.length === topics.length;
            const allQuizzesComplete = completedQuizzesInLesson.length === quizzes.length;
            const lessonAutoComplete = (topics.length > 0 || quizzes.length > 0) &&
                allTopicsComplete &&
                allQuizzesComplete;
            // If lesson has no topics or quizzes, rely on explicit lesson completion
            const effectiveLessonCompleted = lessonAutoComplete ||
                (topics.length === 0 && quizzes.length === 0 && lessonCompleted);
            // For display purposes, calculate based on topic/quiz completion only
            const totalContentItems = topics.length + quizzes.length;
            const completedContentItems = completedTopicsInLesson.length + completedQuizzesInLesson.length;
            const lessonPercentage = totalContentItems > 0
        ...
        const completedItems = completedLessonsCount + completedTopicsCount + completedQuizzesCount;
