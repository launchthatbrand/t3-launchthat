...
export const markItemComplete = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await updateCourseProgress(ctx, args.userId, args.courseId, args.itemId, args.itemType);
        return null;
    },
});
/**
...
export const startItem = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await updateLastAccessed(ctx, args.userId, args.courseId, args.itemId, args.itemType);
        return null;
    },
});
/**
 ...
 */
async function updateCourseProgress(ctx, userId, courseId, completedItemId, completedItemType) {
    const now = Date.now();
    ...
    if (completedItemType === "lesson") {
        if (!updatedLessons.completed.includes(completedItemId)) {
            updatedLessons.completed.push(completedItemId);
            updatedCompleted++;
        }
    }
        ...
        if (topic && "lessonId" in topic && topic.lessonId) {
            ...
            if (!updatedTopics[lessonId].includes(completedItemId)) {
                updatedTopics[lessonId].push(completedItemId);
                updatedCompleted++;
            }
        }
    }
 ...
 */
async function updateLastAccessed(ctx, userId, courseId, itemId, itemType) {
    const now = Date.now();
    let courseProgress = await ctx.db
        .query("courseProgress")
        .withIndex("by_user_course", (q) => q.eq("userId", userId).eq("courseId", courseId))
 ...
 */
async function initializeCourseProgress(ctx, userId, courseId) {
    const now = Date.now();
    ...
    const topics = await filter(ctx.db.query("topics"), (topic) => lessonIds.includes(topic.lessonId)).collect();
...
async function markItemCompleteHelper(ctx, args) {
    const now = Date.now();
    // Update or create individual item progress
    const existingProgress = await ctx.db
        .query("progress")
