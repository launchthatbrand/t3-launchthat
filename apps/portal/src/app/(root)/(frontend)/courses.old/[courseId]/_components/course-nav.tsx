...
export function CourseNav({ config }: { config: DocsConfig }) {
    ...
    <div className="flex flex-col gap-6">
      {items.map((item, index) => (
        <div key={index} className="flex flex-col gap-1">
          ...
          {item?.items?.length && (
            <CourseNavItems items={item.items} pathname={pathname} />
          )}
        </div>
      ))}
    </div>
    ...
    <div className="grid grid-flow-row auto-rows-max gap-0.5 text-sm">
      {items.map((item, index) =>
        item.href && !item.disabled ? (
          <CourseNavItem key={index} item={item} pathname={pathname} />
        ) : (
          <span
            key={index}
            className={cn(
