...
export function FeedStream({
  ...
  switch (feedType) {
    case "universal":
      queryFn = api.socialfeed.queries.getUniversalFeed;
      break;
    case "personalized":
      queryFn = api.socialfeed.queries.getPersonalizedFeed;
      queryArgs.userId = filters.userId as Id<"users">;
      break;
    case "group":
      queryFn = api.socialfeed.queries.getGroupFeed;
      queryArgs.groupId = targetId as Id<"groups">;
      break;
    case "profile":
      queryFn = api.socialfeed.queries.getUserProfileFeed;
  ...
  const queryResult = useQuery(queryFn, queryArgs);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (newItems.length > 0 && newItems.length >= limit) {
        // Use the ID of the last item as cursor for next query
