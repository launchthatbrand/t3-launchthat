...
export function ReminderSettings({
  ...
  const [reminders, setReminders] = useState<
    Array<{ type: string; minutesBefore: number }>
  >([]);
  ...
  const handleReminderTypeChange = (index: number, type: string) => {
    const newReminders = [...reminders];
    newReminders[index].type = type as "email" | "notification" | "sms";
    setReminders(newReminders);
  };

  const handleReminderTimeChange = (index: number, minutesBefore: string) => {
    const newReminders = [...reminders];
    newReminders[index].minutesBefore = parseInt(minutesBefore);
    setReminders(newReminders);
  };
  ...
  const formatReminderText = () => {
    ...
    if (reminders.length === 1) {
      const { minutesBefore } = reminders[0];
