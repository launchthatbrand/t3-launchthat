...
const createAuthNetTransactionHttpAction = httpAction(async (ctx, request) => {
    try {
        ...
        if (!opaqueData || // Explicitly check if opaqueData exists
            ...
            lineItems.length === 0) {
            return new Response(JSON.stringify({ error: "Invalid request body payload" }), {
                status: 400,
                headers: {
                    ...createCorsHeaders(),
        ...
        await ctx.scheduler.runAfter(0, internal.actions.payments.internalCreateAuthNetTransaction, {
...
http.route({
    ...
    handler: httpAction(async (ctx, req) => {
        try {
            ...
            const lessonId = await ctx.runMutation(api.lms.lessons.mutations.createViaWebhook, json);
