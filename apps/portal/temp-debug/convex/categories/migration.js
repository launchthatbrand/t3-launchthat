    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            try {
                ...
                warnings.push("MediaCategories table not found - skipping media category migration");
            }
            ...
            try {
                ...
                warnings.push("ProductCategories table not found - skipping product category migration");
            }
            ...
            try {
                ...
                warnings.push("DownloadCategories table not found - skipping download category migration");
            }
            ...
            try {
                ...
                warnings.push("Posts table not found - skipping CMS category migration");
            }
                    ...
                    for (const item of mediaItems) {
                        if (item.categories && item.categories.length > 0) {
                            ...
                            for (const categoryName of item.categories) {
                                const globalCategory = await ctx.runQuery(api.categories.getCategoriesByPostType, {
                                    postTypes: ["media"],
                                });
                                const matchingCategory = globalCategory.find((cat) => cat.name === categoryName);
                                if (matchingCategory) {
...
export const previewCategoryMigration = query({
    ...
    handler: async (ctx) => {
        ...
        try {
            ...
            itemsToUpdate.posts = posts.filter((p) => p.category && p.category !== "Uncategorized").length;
        }
        catch {
        ...
        try {
            const mediaItems = await ctx.db.query("mediaItems").collect();
            itemsToUpdate.mediaItems = mediaItems.filter((item) => item.categories && item.categories.length > 0).length;
        }
        catch {
...
export const cleanupOldCategoryTables = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            errors.push("Table cleanup must be done by removing tables from schema files and redeploying");
            return {
                success: true,
                deletedTables,
                errors,
...
export const validateMigration = query({
    ...
    handler: async (ctx) => {
        ...
        try {
            const mediaItems = await ctx.db.query("mediaItems").collect();
            itemsWithGlobalCategories.mediaItems = mediaItems.filter((item) => item.categoryIds && item.categoryIds.length > 0).length;
        }
        catch {
