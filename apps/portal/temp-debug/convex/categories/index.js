...
export const getCategoriesByPostType = query({
    ...
    handler: async (ctx, args) => {
        ...
        categories = categories.filter((category) => category.postTypes.some((postType) => args.postTypes.includes(postType)));
...
export const getCategoryTree = query({
    ...
    handler: async (ctx, args) => {
        ...
        const categories = await ctx.runQuery(api.categories.getCategoriesByPostType, {
            postTypes: args.postTypes,
            isActive: args.isActive,
            isVisible: args.isVisible,
        });
        // Build tree structure
        const categoryMap = new Map(categories.map((cat) => [cat._id, { ...cat, children: [] }]));
...
export const searchCategories = query({
    ...
    handler: async (ctx, args) => {
        const results = await ctx.db
            .query("categories")
            .withSearchIndex("search_categories", (q) => q.search("name", args.searchTerm))
            .take(args.limit ?? 20);
        // Filter by post types if specified
        let filteredResults = results;
        if (args.postTypes && args.postTypes.length > 0) {
            filteredResults = results.filter((category) => category.postTypes.some((postType) => args.postTypes.includes(postType)));
        }
...
export { migrateCategoriesToGlobal, previewCategoryMigration, cleanupOldCategoryTables, validateMigration, } from "./migration";
