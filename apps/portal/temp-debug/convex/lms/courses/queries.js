...
export const getCourseById = query({
    ...
    returns: v.union(v.null(), v.object({
        ...
        courseStructure: v.optional(v.array(v.object({ lessonId: v.id("lessons") }))),
...
export const listCourses = query({
    ...
    handler: async (ctx, args) => {
        if (args.organizationId && args.isPublished !== undefined) {
            ...
            return await ctx.db
                .query("courses")
                .withIndex("by_organization_published", (q) => q
...
export const searchCourses = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.organizationId) {
            searchQuery = searchQuery.filter((q) => q.eq(q.field("organizationId"), args.organizationId));
        }
        if (args.isPublished !== undefined) {
...
export const listPublishedCourses = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.searchTitle && args.searchTitle.trim().length > 0) {
            let searchQuery = ctx.db
                .query("courses")
                .withSearchIndex("search_title", (q) => q.search("title", args.searchTitle.trim()));
            // Always filter to published courses
            searchQuery = searchQuery.filter((q) => q.eq(q.field("isPublished"), true));
            ...
            if (args.organizationId) {
                ...
                return await ctx.db
                    .query("courses")
                    .withIndex("by_organization_published", (q) => q.eq("organizationId", args.organizationId).eq("isPublished", true))
                    .take(args.limit ?? 50);
            }
            else {
