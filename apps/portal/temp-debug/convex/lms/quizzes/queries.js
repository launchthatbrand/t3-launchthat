...
export const getAvailable = query({
    ...
    returns: v.array(v.object({
        ...
        questions: v.optional(v.array(v.object({
            type: v.union(v.literal("single-choice"), v.literal("multiple-choice"), v.literal("true-false")),
    ...
    handler: async (ctx, args) => {
        ...
        const quizzes = await filter(ctx.db.query("quizzes"), (quiz) => quiz.topicId !== args.topicId).collect();
        return quizzes;
    },
});
/**
...
export const listQuizzes = query({
    ...
    handler: async (ctx, args) => {
        ...
        const filtered = args.searchTitle
            ? allQuizzes.filter((q) => q.title.toLowerCase().includes(args.searchTitle.toLowerCase()))
            : allQuizzes;
