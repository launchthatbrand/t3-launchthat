 ...
 */
export async function notifyReaction(ctx, args) {
    ...
    if (!feedItem || feedItem.deleted)
        return;
    // Only send notification if the reactor is not the creator
    if (feedItem.creatorId === reactorId)
        return;
 ...
 */
export async function notifyComment(ctx, args) {
 ...
 */
async function notifyDirectComment(ctx, args) {
    ...
    if (feedItem.creatorId === commenterId)
        return;
    // Get user information for better notifications
 ...
 */
async function notifyCommentReply(ctx, args) {
    ...
    if (parentComment.userId === commenterId)
        return;
    // Get user information for better notifications
    const commenter = await ctx.db.get(commenterId);
    if (!commenter)
 ...
 */
export async function notifyMentions(ctx, args) {
    ...
    for (const username of usernames) {
        ...
        if (!mentionedUser || mentionedUser._id === mentionerId)
            continue;
        // Create notification
        await ctx.db.insert("notifications", {
 ...
 */
export async function notifyShare(ctx, args) {
    ...
    if (!originalContent || originalContent.deleted)
        return;
    // Only send notification if the sharer is not the original creator
    if (originalContent.creatorId === sharerId)
        return;
    // Get user information for better notifications
 ...
 */
export async function notifyFollowers(ctx, args) {
    ...
    const userFollowers = subscriptions.filter((sub) => sub.followType === "user" && sub.followId === creatorId);
