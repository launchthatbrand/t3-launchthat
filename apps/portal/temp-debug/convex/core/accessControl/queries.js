...
export const getDownloadAccessList = query({
    ...
    handler: async (ctx, args) => {
        ...
        const hasManagePermission = await hasDownloadPermission(ctx, DownloadPermissions.EDIT_ANY);
        ...
        if (download.requiredProductId) {
            ...
            for (const order of purchasingUsers) {
                ...
                const hasRequiredProduct = order.items.some((item) => item.productId === download.requiredProductId);
                if (hasRequiredProduct && order.userId) {
                    // Skip if already in the list
                    if (result.some((u) => u._id === order.userId))
                        continue;
        ...
        if (download.requiredCourseId) {
            ...
            for (const enrollment of enrolledUsers) {
                // Skip if already in the list
                if (result.some((u) => u._id === enrollment.userId))
                    continue;
