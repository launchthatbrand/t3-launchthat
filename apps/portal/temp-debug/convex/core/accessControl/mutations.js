...
export const assignDownloadRole = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!currentUser || currentUser.role !== "admin") {
            throw new ConvexError("Permission denied: Only admins can assign download roles.");
        }
        if (!Object.values(DownloadRoles).includes(args.roleToAssign)) {
...
export const grantDownloadAccess = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const hasEditPermission = await hasDownloadPermission(ctx, DownloadPermissions.EDIT_ANY);
