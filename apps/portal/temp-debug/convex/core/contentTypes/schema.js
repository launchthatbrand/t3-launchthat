...
const fieldDefaultValue = v.optional(v.union(v.string(), v.number(), v.boolean(), v.null()));
const fieldValidationRules = v.optional(v.union(v.record(v.string(), v.union(v.string(), v.number(), v.boolean(), v.array(v.union(v.string(), v.number())), v.object({
...
const fieldOptions = v.optional(v.union(
...
v.record(v.string(), v.union(v.string(), v.number(), v.boolean(), v.array(v.union(v.string(), v.number(), v.object({
    label: v.string(),
    value: v.union(v.string(), v.number()),
}))))), v.null()));
const fieldUiConfig = v.optional(v.union(v.record(v.string(), v.union(v.string(), v.number(), v.boolean(), v.object({
...
export const contentTypeFieldsTable = defineTable({
    ...
    // Field Configuration
    type: v.union(v.literal("text"), v.literal("textarea"), v.literal("number"), v.literal("boolean"), v.literal("date"), v.literal("datetime"), v.literal("select"), v.literal("multiselect"), v.literal("relation"), v.literal("file"), v.literal("image"), v.literal("richtext"), v.literal("json"), v.literal("array"), v.string()), // Specific field types with fallback for extensibility
