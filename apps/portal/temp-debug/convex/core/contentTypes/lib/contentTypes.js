 ...
 */
export async function getContentTypeBySlug(ctx, slug) {
    return await ctx.db
        .query("contentTypes")
 ...
 */
export async function getContentTypeById(ctx, id) {
    return await ctx.db.get(id);
}
/**
 * Get all fields for a content type
 */
export async function getContentTypeFields(ctx, contentTypeId, includeSystem = true) {
    const query = ctx.db
 ...
 */
export async function getFieldByKey(ctx, contentTypeId, key) {
    return await ctx.db
        .query("contentTypeFields")
        .withIndex("by_contentType_key", (q) => q.eq("contentTypeId", contentTypeId).eq("key", key))
        .unique();
}
/**
 ...
 */
export async function createSystemFields(ctx, contentTypeId) {
    ...
    const promises = systemFields.map((field) => ctx.db.insert("contentTypeFields", field));
 ...
 */
export async function updateFieldCount(ctx, contentTypeId) {
    ...
    const fields = await ctx.db
        .query("contentTypeFields")
        .withIndex("by_contentType_isSystem", (q) => q.eq("contentTypeId", contentTypeId).eq("isSystem", false))
        .collect();
 ...
 */
export function generateTableName(slug) {
 ...
 */
export async function updateEntryCount(ctx, contentTypeSlug) {
    ...
    try {
        switch (contentTypeSlug) {
            ...
            default: {
                ...
                console.log(`No specific count handler for content type: ${contentTypeSlug}`);
                break;
            }
        }
    }
