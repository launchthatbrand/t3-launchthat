...
import { logError, throwForbidden, throwNotFound, throwUnauthorized, } from "../shared/errors";
...
export const internalEnsureUser = internalMutation({
    ...
    handler: async (ctx) => {
        ...
        console.log(`--- internalEnsureUser: Identity found for token ${identity.tokenIdentifier}, subject ${identity.subject} ---`);
        ...
        const existingUser = await ctx.db
            ...
            .first();
        if (existingUser) {
            console.log(`--- internalEnsureUser: User ${identity.subject} already exists with ID ${existingUser._id}. Checking for updates... ---`);
            // Check if name or image needs updating
            const newName = identity.name ?? identity.nickname ?? "Unnamed User";
            const newImage = typeof identity.picture === "string" ? identity.picture : undefined; // Normalize to string | undefined
            const updates = {}; // image can be undefined if not present
            ...
            if (Object.keys(updates).length > 0) {
                try {
                    ...
                    console.log(`--- internalEnsureUser: Updated user ${existingUser._id} with new data:`, updates);
                }
                catch (error) {
        ...
        console.log(`--- internalEnsureUser: Creating new user for ${identity.subject}... ---`);
        ...
        if (adminEmail && identity.email && identity.email === adminEmail) {
            console.log(`--- internalEnsureUser: Assigning admin role to user ${identity.email} ---`);
            userRole = "admin";
        }
        ...
        if (!userEmail) {
            console.warn(`--- internalEnsureUser: User ${identity.subject} has no email address. Cannot assign role based on email. ---`);
            userRole = "user";
        }
        ...
        try {
            ...
            console.log(`--- internalEnsureUser: New user ${userId} created with role: ${userRole} ---`);
            return userId;
        }
        catch (error) {
...
export const createOrGetUser = mutation({
    ...
    handler: async (ctx) => {
        ...
        const userId = await ctx.runMutation(internal.users.mutations.internalEnsureUser, {});
        console.log("--- createOrGetUser (public wrapper) FINISHED, internalEnsureUser returned: ", userId);
...
export const updateUser = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.data.email !== undefined)
            update.email = args.data.email;
        if (args.data.role !== undefined)
            update.role = args.data.role;
        // Update the user
