...
export const createGroup = mutation({
    args: {
        ...
        privacy: v.union(v.literal("public"), v.literal("private"), v.literal("restricted")),
...
export const updateGroup = mutation({
    args: {
        ...
        coverImage: v.optional(v.string()),
        categoryTags: v.optional(v.array(v.string())),
        headerItems: v.optional(v.array(v.object({
            id: v.string(),
            title: v.optional(v.string()),
            excerpt: v.optional(v.string()),
            imageUrl: v.string(),
            template: v.union(v.literal("inline"), v.literal("stacked"), v.literal("overlay")),
    ...
    handler: async (ctx, args) => {
        ...
        const isAdminOrMod = await isGroupAdminOrModerator(ctx, user._id, args.groupId);
        if (!isAdminOrMod) {
            throw new ConvexError({
                code: 403,
        ...
        if (args.headerItems !== undefined)
            updates.headerItems = args.headerItems;
    ...
    handler: async (ctx, args) => {
        ...
        if (membership.role === "admin") {
            const otherAdmins = await ctx.db
                ...
                .filter((q) => q.and(q.eq(q.field("role"), "admin"), q.neq(q.field("userId"), user._id)))
                .collect();
            if (otherAdmins.length === 0) {
                throw new ConvexError({
                    code: 400,
                    message: "Cannot leave group as the last admin. Transfer ownership first.",
                });
            }
        }
...
export const inviteToGroup = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingMembership = await ctx.db
            .query("groupMembers")
            .withIndex("by_group_user", (q) => q.eq("groupId", args.groupId).eq("userId", args.invitedUserId))
            .first();
        if (existingMembership) {
            if (existingMembership.status === "active") {
                throw new ConvexError({
        ...
        const existingInvitation = await ctx.db
            ...
            .withIndex("by_group_user", (q) => q.eq("groupId", args.groupId).eq("invitedUserId", args.invitedUserId))
            .first();
...
export const removeGroupMember = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const memberToRemove = await ctx.db
            .query("groupMembers")
            .withIndex("by_group_user", (q) => q.eq("groupId", args.groupId).eq("userId", args.memberId))
            .first();
        if (!memberToRemove) {
            throw new ConvexError({
                code: 404,
        ...
        if (userMembership.role === "moderator" &&
        ...
        if (memberToRemove.role === "admin") {
            const otherAdmins = await ctx.db
                .query("groupMembers")
                .withIndex("by_group", (q) => q.eq("groupId", args.groupId))
                .filter((q) => q.and(q.eq(q.field("role"), "admin"), q.neq(q.field("userId"), args.memberId)))
                .collect();
            if (otherAdmins.length === 0) {
                throw new ConvexError({
                    code: 400,
    ...
    handler: async (ctx, args) => {
        ...
        if (memberToUpdate.role === "admin" && args.newRole !== "admin") {
            ...
            if (otherAdmins.length === 0) {
                throw new ConvexError({
                    code: 400,
                    message: "Cannot demote the last admin. Transfer ownership to another member first.",
                });
            }
        }
