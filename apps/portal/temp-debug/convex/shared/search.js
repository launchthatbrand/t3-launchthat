 ...
 */
export const createSearchFilter = (searchText, fields) => {
    ...
    return (item) => {
        ...
        return fields.some((field) => {
            const fieldValue = item[field] !== undefined && item[field] !== null
                ? String(item[field])
                : "";
 ...
 */
export const scoreSearchResult = (item, searchText, fields) => {
    if (!searchText)
        return 0;
    const normalizedSearch = normalizeSearchText(searchText);
    const searchTerms = normalizedSearch.split(/\s+/).filter(Boolean);
    ...
    fields.forEach((fieldDef) => {
        ...
        const fieldValue = item[fieldName] !== undefined && item[fieldName] !== null
