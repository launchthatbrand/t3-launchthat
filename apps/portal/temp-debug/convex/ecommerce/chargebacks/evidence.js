...
export const getChargebackEvidence = query({
    ...
    returns: v.array(v.object({
        ...
        chargebackId: v.id("chargebacks"),
        documentType: v.union(v.literal("receipt"), v.literal("shipping_proof"), v.literal("customer_communication"), v.literal("refund_policy"), v.literal("terms_of_service"), v.literal("product_description"), v.literal("customer_signature"), v.literal("billing_statement"), v.literal("transaction_history"), v.literal("dispute_response"), v.literal("audit_log"), v.literal("other")),
        title: v.string(),
        description: v.optional(v.string()),
        fileStorageId: v.optional(v.id("_storage")),
        textContent: v.optional(v.string()),
        url: v.optional(v.string()),
        processorRelevance: v.optional(v.object({
            stripe: v.optional(v.boolean()),
            authorizeNet: v.optional(v.boolean()),
            paypal: v.optional(v.boolean()),
            square: v.optional(v.boolean()),
        })),
        submissionStatus: v.union(v.literal("draft"), v.literal("ready"), v.literal("submitted"), v.literal("accepted"), v.literal("rejected")),
    ...
    handler: async (ctx, args) => {
        const query = ctx.db
            ...
            .withIndex("by_chargebackId", (q) => q.eq("chargebackId", args.chargebackId));
        const result = await query.order("desc").collect();
        // Apply additional filters after collecting
        let filteredResult = result;
        if (args.documentType) {
            filteredResult = filteredResult.filter((item) => item.documentType === args.documentType);
        }
        if (args.submissionStatus) {
            filteredResult = filteredResult.filter((item) => item.submissionStatus === args.submissionStatus);
        }
...
export const updateEvidence = mutation({
    args: {
        ...
        submissionStatus: v.optional(v.union(v.literal("draft"), v.literal("ready"), v.literal("submitted"), v.literal("accepted"), v.literal("rejected"))),
        importance: v.optional(v.union(v.literal("critical"), v.literal("high"), v.literal("medium"), v.literal("low"))),
        tags: v.optional(v.array(v.string())),
        metadata: v.optional(v.record(v.string(), v.union(v.string(), v.number(), v.boolean()))),
    },
    returns: v.null(),
