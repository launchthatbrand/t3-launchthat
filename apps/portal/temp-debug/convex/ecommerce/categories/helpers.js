 ...
 */
export async function generateCategorySlug(ctx, name, excludeId) {
    const baseSlug = name
        .toLowerCase()
        .replace(/\s+/g, "-")
        .replace(/[^a-z0-9-]/g, "");
 ...
 */
export async function updateChildrenPathsAndLevels(ctx, parentIdToUpdateChildrenOf, newParentLevel, newParentPath) {
    ...
    for (const child of children) {
        ...
        await updateChildrenPathsAndLevels(ctx, child._id, newChildLevel, newChildPath);
    }
}
