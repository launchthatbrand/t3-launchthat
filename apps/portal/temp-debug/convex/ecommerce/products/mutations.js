...
export const createProduct = mutation({
    args: {
        ...
        sku: v.string(),
        stockStatus: v.optional(v.union(v.literal("in_stock"), v.literal("out_of_stock"))),
    ...
    handler: async (ctx, args) => {
        ...
        let primaryCategoryId = args.primaryCategoryId;
    ...
    handler: async (ctx, args) => {
        ...
        if (updateData.name || customSlug) {
            if (customSlug) {
                ...
                updates.slug = await generateUniqueSlug(ctx.db, "products", sanitizedSlug, productId);
            }
            else if (updateData.name) {
                updates.slug = await generateUniqueSlug(ctx.db, "products", updateData.name, productId);
            }
        }
        ...
        if (updateData.taxable !== undefined)
            updates.taxable = updateData.taxable;
