    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            if (!mockUser) {
                ...
                const mockUserResult = await ctx.runMutation(api.users.mockData.createMockUser, {});
            ...
            for (let i = 0; i < orderCount; i++) {
                const orderResult = await ctx.runMutation(api.ecommerce.orders.mockData.createMockOrder, {});
            ...
            const newTransferId = await ctx.db.insert("transfers", {
                ...
                expectedArrival: status === "pending" || status === "in_transit"
                    ? expectedArrival
                    : undefined,
                completedAt,
                fees: transferFees,
            ...
            throw new Error(`Failed to create mock transfer: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    },
});
