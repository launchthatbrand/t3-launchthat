...
import { requireAdmin, requireAuthentication, } from "../../lib/permissions/requirePermission";
 ...
 */
async function getCustomCheckoutBySlugInternal(ctx, slug) {
...
export const createCustomCheckoutSession = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const checkout = await getCustomCheckoutBySlugInternal(ctx, args.checkoutSlug);
        if (!checkout) {
            throw new Error(`Custom checkout "${args.checkoutSlug}" not found or inactive`);
        }
...
export const createCustomCheckoutFromContentType = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            ...
            .first();
        if (existing) {
            throw new Error(`A custom checkout with slug "${args.slug}" already exists`);
        }
...
export const updateCustomCheckout = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.productIds)
            updateData.productIds = args.productIds;
        ...
        if (args.successUrl !== undefined)
            updateData.successUrl = args.successUrl;
        if (args.cancelUrl !== undefined)
            updateData.cancelUrl = args.cancelUrl;
        if (args.status)
