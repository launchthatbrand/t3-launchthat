 ...
 */
export const hasPermission = async (ctx, permissionKey, options) => {
    try {
 ...
 */
export const hasResourceAccess = async (ctx, permissionKey, resourceOwnerId, options) => {
    try {
...
export const isAdmin = async (ctx) => {
    try {
        ...
        const adminRoles = ["admin", "administrator"];
        if (adminRoles.includes(roleNormalized))
            return true;
