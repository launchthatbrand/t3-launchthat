...
import { notifyComment, notifyFollowers, notifyMentions, notifyReaction, notifyShare, } from "../notifications/lib/feedNotifications";
import { generateRecommendations, markRecommendationInteracted, markRecommendationSeen, } from "./lib/recommendationEngine";
import { updateTrendingMetrics } from "./lib/trendingAlgorithm";
// Helper functions for processing hashtags and mentions
async function processMentionsAndHashtags(ctx, text, creatorId, feedItemId) {
...
export const createPost = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const { mentions, mentionedUserIds, hashtags } = await processMentionsAndHashtags(ctx, args.content, args.creatorId, feedItemId);
...
export const addReaction = mutation({
    args: {
        ...
        reactionType: v.union(v.literal("like"), v.literal("love"), v.literal("celebrate"), v.literal("support"), v.literal("insightful"), v.literal("curious")),
    },
    ...
    handler: async (ctx, args) => {
        ...
        const existingReaction = await ctx.db
            .query("reactions")
            .withIndex("by_user_and_item", (q) => q.eq("userId", args.userId).eq("feedItemId", args.feedItemId))
            .first();
...
export const addComment = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const { mentions, mentionedUserIds, hashtags } = await processMentionsAndHashtags(ctx, args.content, args.userId, args.feedItemId);
...
export const markRecommendationAsInteracted = mutation({
    args: {
        userId: v.id("users"),
        contentId: v.id("feedItems"),
        reaction: v.optional(v.union(v.literal("like"), v.literal("dislike"), v.literal("neutral"))),
    },
    returns: v.boolean(),
    handler: async (ctx, args) => {
        await markRecommendationInteracted(ctx, args.userId, args.contentId, args.reaction);
        return true;
    },
});
/**
