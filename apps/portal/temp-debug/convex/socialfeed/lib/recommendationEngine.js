 ...
 */
export async function generateRecommendations(ctx, userId, limit = 10) {
    ...
    const followedUsers = await ctx.db
        .query("subscriptions")
        .withIndex("by_user_and_type", (q) => q.eq("userId", userId).eq("followType", "user"))
        .collect();
    ...
    const userReactions = await ctx.db
        ...
        .order("desc")
        .take(50);
    const engagedContentIds = userReactions.map((reaction) => reaction.feedItemId);
    // Get hashtags from content the user has engaged with
    const engagedContent = await Promise.all(engagedContentIds.map(async (id) => ctx.db.get(id)));
    ...
    if (followedTopicIds.length > 0) {
        ...
        const topics = await Promise.all(followedTopicIds.map(async (id) => ctx.db.get(id)));
    ...
    const allCandidateContent = [
        ...topicContent,
        ...userContent,
        ...(await Promise.all(trendingContentIds.map(async (id) => ctx.db.get(id)))),
    ].filter(Boolean);
    ...
    for (const content of allCandidateContent) {
        ...
        const existingRecommendation = await ctx.db
            ...
            .withIndex("by_user_and_content", (q) => q.eq("userId", userId).eq("contentId", content._id))
            .first();
        if (existingRecommendation)
            continue;
        // Calculate a relevance score and determine reason
        let score = 1.0;
        let reason = "newContent";
        let context = undefined;
        // Check if from followed topic
        if (content.hashtags && content.hashtags.length > 0) {
            const matchingTags = content.hashtags.filter((tag) => relatedHashtags.includes(tag));
        ...
        // Check if trending
        const trendingScore = trendingContent.find((tc) => tc.contentId === content._id)
            ?.trendingScore ?? 0;
        if (trendingScore > 0) {
 ...
 */
export async function findSimilarUsers(ctx, userId, limit = 5) {
    // Get user's engagement
    const userReactions = await ctx.db
        .query("reactions")
        .withIndex("by_user", (q) => q.eq("userId", userId))
        .collect();
    const userEngagedContent = new Set(userReactions.map((r) => r.feedItemId.toString()));
 ...
 */
export async function markRecommendationSeen(ctx, userId, contentId) {
    const recommendation = await ctx.db
        .query("contentRecommendations")
        .withIndex("by_user_and_content", (q) => q.eq("userId", userId).eq("contentId", contentId))
        .first();
    if (recommendation) {
 ...
 */
export async function markRecommendationInteracted(ctx, userId, contentId, reaction) {
    const recommendation = await ctx.db
        .query("contentRecommendations")
        .withIndex("by_user_and_content", (q) => q.eq("userId", userId).eq("contentId", contentId))
        .first();
