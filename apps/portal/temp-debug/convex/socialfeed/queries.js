...
const commentReturnType = v.object({
    ...
    parentId: v.union(v.id("feedItems"), v.id("courses"), v.id("lessons"), v.id("topics"), v.id("quizzes"), v.id("posts"), v.id("downloads"), v.id("helpdeskArticles")),
    parentType: v.union(v.literal("feedItem"), v.literal("course"), v.literal("lesson"), v.literal("topic"), v.literal("quiz"), v.literal("post"), v.literal("download"), v.literal("helpdeskArticle")),
    userId: v.id("users"),
    content: v.string(),
...
async function enrichComments(ctx, page) {
    ...
    const commentsWithCounts = await Promise.all(page.map(async (comment) => {
        // Count replies
...
const feedItemReturnType = v.object({
    _id: v.id("feedItems"),
    _creationTime: v.number(),
    contentType: v.union(v.literal("post"), v.literal("share"), v.literal("comment")),
    creatorId: v.id("users"),
    content: v.string(),
    mediaUrls: v.optional(v.array(v.string())),
    visibility: v.union(v.literal("public"), v.literal("private"), v.literal("group")),
    originalContentId: v.optional(v.id("feedItems")),
    moduleType: v.optional(v.union(v.literal("blog"), v.literal("course"), v.literal("group"), v.literal("event"))),
...
async function enrichFeedItems(ctx, page) {
    // Early return if no items
    if (page.length === 0) {
        return [];
    }
    ...
    const creators = await Promise.all(creatorIds.map(async (id) => {
        const user = await ctx.db.get(id);
        return user
            ? {
                _id: user._id,
    ...
    const itemsWithCounts = await Promise.all(page.map(async (item) => {
        // Count reactions
        ...
        const commentsCount = await ctx.db
            .query("comments")
            .withIndex("by_parent", (q) => q.eq("parentId", item._id).eq("parentType", "feedItem"))
            .collect()
...
export const getPersonalizedFeed = query({
    ...
    handler: async (ctx, args) => {
        ...
        const subscriptions = await ctx.db
            .query("subscriptions")
            .withIndex("by_user_and_type", (q) => q.eq("userId", args.userId).eq("followType", "user"))
            .collect();
        ...
        const { page, continueCursor, isDone } = await ctx.db
            ...
            return q.or(q.eq(q.field("visibility"), "public"), q.and(q.eq(q.field("visibility"), "private"), q.or(q.eq(q.field("creatorId"), args.userId), 
...
export const getGroupFeed = query({
    ...
    handler: async (ctx, args) => {
        ...
        const { page, continueCursor, isDone } = await ctx.db
            .query("feedItems")
            .withIndex("by_module", (q) => q.eq("moduleType", "group").eq("moduleId", args.groupId))
            .order("desc")
            .paginate(args.paginationOpts);
        const enrichedFeedItems = await enrichFeedItems(ctx, page);
        return { page: enrichedFeedItems, continueCursor, isDone };
...
export const getUserProfileFeed = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (!isSameUser) {
            ...
            feedItemsQuery = feedItemsQuery.filter((q) => q.eq(q.field("visibility"), "public"));
        }
...
export const getComments = query({
    ...
    handler: async (ctx, args) => {
        ...
        let queryBuilder = ctx.db
            .query("comments")
            .withIndex("by_parent", (q) => q.eq("parentId", args.parentId).eq("parentType", args.parentType))
        ...
        const { page, continueCursor, isDone } = await queryBuilder.paginate(args.paginationOpts);
...
export const getReplies = query({
    ...
    handler: async (ctx, args) => {
        ...
        let queryBuilder = ctx.db
            .query("comments")
            .withIndex("by_parentCommentId", (q) => q.eq("parentCommentId", args.parentCommentId));
        // Apply sorting
...
export const searchUsersForMentions = query({
    ...
    handler: async (ctx, args) => {
        ...
        const users = await ctx.db
            .query("users")
            .withSearchIndex("search_name_username", (q) => q.search("name", args.query).search("username", args.query))
            .take(args.limit ?? 10);
...
export const getRecommendedContent = query({
    ...
    handler: async (ctx, args) => {
        ...
        ...recommendations.similarUserIds.map((id) => q.eq(q.field("creatorId"), id)), 
...
export const getTopics = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.query) {
            ...
            topicsQuery = topicsQuery.filter((q) => q.or(q.contains(q.lower(q.field("tag")), searchLower), q.contains(q.lower(q.field("description") ?? ""), searchLower)));
        }
...
export const getUserFollowedTopics = query({
    ...
    handler: async (ctx, args) => {
        ...
        const topics = await Promise.all(topicIds.map(async (id) => await ctx.db.get(id)));
...
export const getTopicSuggestions = query({
    ...
    handler: async (ctx, args) => {
        ...
        const userFollows = await ctx.db
            ...
            .withIndex("by_user", (q) => q.eq("userId", args.userId))
            .collect();
        const followedTopicIds = new Set(userFollows.map((f) => f.topicId.toString()));
        ...
        const engagedContent = await Promise.all(contentIds.map(async (id) => await ctx.db.get(id)));
        // Extract hashtags from engaged content
        const engagedHashtags = new Set();
        ...
        const similarUserFollows = await Promise.all(similarUserIds.map(async (id) => ctx.db
            .query("topicFollows")
            .withIndex("by_user", (q) => q.eq("userId", id))
            .collect()));
        // Flatten and get unique topic IDs
        const similarUserTopicIds = new Set();
        ...
        if (engagedHashtags.size > 0) {
            const engagedTopics = await ctx.db
                .query("hashtags")
                .withIndex("by_isTopic", (q) => q.eq("isTopic", true))
                .filter((q) => q.and(q.not(followedTopicIds.has(q.field("_id").toString())), engagedHashtags.has(q.field("tag"))))
                .take(limit);
        ...
        if (similarUserTopicIds.size > 0 && suggestions.length < limit) {
            ...
            const similarUserTopics = await Promise.all(Array.from(similarUserTopicIds)
                .filter((id) => !followedTopicIds.has(id))
                .slice(0, remainingCount)
                .map(async (id) => await ctx.db.get(id)));
            suggestions.push(...similarUserTopics.filter((t) => t !== null));
        }
        ...
        if (suggestions.length < limit) {
            ...
            const additionalPopularTopics = popularTopics
                .filter((topic) => !followedTopicIds.has(topic._id.toString()) &&
...
export const getRecommendedHashtags = query({
    ...
    handler: async (ctx, args) => {
        ...
        const relevantFeedItems = await ctx.db
            .query("feedItems")
            .filter((q) => q.or(q.eq(q.field("creatorId"), args.userId), q.or(followedUserIds.length > 0
            ? q.in(q.field("creatorId"), followedUserIds)
            : q.eq(q.field("creatorId"), args.userId))))
            .collect();
        const allHashtags = relevantFeedItems.flatMap((item) => item.hashtags ?? []);
        const hashtagCounts = new Map();
        for (const hashtag of allHashtags) {
            hashtagCounts.set(hashtag, (hashtagCounts.get(hashtag) ?? 0) + 1);
        }
        const sortedHashtags = [...hashtagCounts.entries()].sort((a, b) => b[1] - a[1]);
        const recommendedHashtagNames = sortedHashtags
            .slice(0, args.limit * 2)
            .map(([hashtag]) => hashtag);
        // Fetch full hashtag documents for the recommended names
        const recommendedHashtagDocs = await Promise.all(recommendedHashtagNames.map(async (hashtagName) => {
            const hashtagDoc = await ctx.db
                .query("hashtags")
