...
export const getConnections = query({
    ...
    handler: async (ctx, args) => {
        ...
        const sourceNodes = await Promise.all(sourceNodeIds.map((id) => ctx.db.get(id)));
        const targetNodes = await Promise.all(targetNodeIds.map((id) => ctx.db.get(id)));
