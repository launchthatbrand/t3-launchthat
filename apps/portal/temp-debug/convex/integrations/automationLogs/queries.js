...
export const getAutomationLogStats = query({
    ...
    handler: async (ctx, args) => {
        ...
        const successfulRuns = runStats.filter((run) => run.isComplete && !run.hasErrors).length;
        const failedRuns = runStats.filter((run) => run.isComplete && run.hasErrors).length;
        ...
        const completedLogsWithDuration = logs.filter((log) => log.duration !== undefined && log.duration > 0);
        const averageDuration = completedLogsWithDuration.length > 0
            ? completedLogsWithDuration.reduce((sum, log) => sum + (log.duration ?? 0), 0) / completedLogsWithDuration.length
            : undefined;
