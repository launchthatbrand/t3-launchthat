...
import { throwForbidden, throwInvalidInput, throwNotFound, } from "../shared/errors";
import { getFileExtension, getFileTypeFromExtension, } from "./lib/fileTypeUtils";
...
export const createFileDownload = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const searchText = prepareSearchText(args.title, args.description, fileName, args.tags);
        ...
        if (!args.skipFeaturedImage) {
            ...
            await ctx.scheduler.runAfter(0, internal.downloads.extractAndAttachFeaturedImage, {
                downloadId,
                storageId: args.storageId,
                fileType,
...
export const extractAndAttachFeaturedImage = action({
    ...
    handler: async (ctx, args) => {
        try {
            ...
            const featuredImageId = await ctx.runAction(internal.downloads.lib.imageExtraction.extractFeaturedImage, {
                storageId: args.storageId,
...
export const generateDownloadUrl = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const hasAccess = await checkDownloadAccess(ctx, userDocId, args.downloadId);
        if (!hasAccess) {
...
export const updateDownload = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const newSearchText = prepareSearchText(updateFields.title ?? currentDownload.title, updateFields.description ?? currentDownload.description, currentDownload.fileName, updateFields.tags ?? currentDownload.tags);
