 ...
 */
async function extractImageFromPdf(ctx, pdfData) {
    try {
        ...
        const uploadId = await ctx.storage.store(new Blob([imageBuffer], { type: "image/png" }));
        return uploadId;
    }
    catch (error) {
 ...
 */
async function extractImageFromPowerPoint(ctx, pptxData) {
    try {
        ...
        try {
            await execPromise(`libreoffice --headless --convert-to png --outdir "${outputDir}" "${tempFilePath}"`, { timeout: 30000 });
        ...
        if (slideImageData) {
            const uploadId = await ctx.storage.store(new Blob([slideImageData], { type: "image/png" }));
            // Clean up temp files
