...
import { throwUnauthorized, } from "../../shared/errors";
/**
...
export async function getAuthenticatedUser(ctx) {
    ...
    const user = await ctx.db
        ...
        .first();
    if (!user) {
        console.error("User not found for tokenIdentifier:", identity.tokenIdentifier);
        throw new ConvexError({
            code: 401,
            message: "User not found. Please complete registration or refresh your session.",
        });
    }
    return user;
}
 ...
 */
export function prepareSearchText(title, description, fileName, tags) {
    const parts = [
        title ?? "",
        description ?? "",
        fileName ?? "",
 ...
 */
export async function checkDownloadAccess(ctx, userId, downloadId) {
    ...
    if (Array.isArray(download.accessibleUserIds) &&
 ...
 */
export async function getGroupDownloads(ctx, groupId, limit = 5) {
