...
import { eventTypeValidator, locationValidator, recurrenceValidator, timestampValidator, visibilityValidator, } from "../../shared/validators";
...
export const updateEvent = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (customSlug) {
            ...
            updates.slug = await generateUniqueSlug(ctx.db, "events", sanitizedSlug, args.eventId);
        }
        else if (updateFields.title && event.title !== updateFields.title) {
            // If title changed, generate a new slug
            updates.slug = await generateUniqueSlug(ctx.db, "events", updateFields.title, args.eventId);
        }
        // Update the event
