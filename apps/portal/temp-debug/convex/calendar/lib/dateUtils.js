 ...
 */
export const validateEventDateRange = (startTime, endTime) => {
 ...
 */
export const getRecurringEventInstances = (event, startDate, endDate) => {
    if (!event.recurrence) {
        return [event]; // Not a recurring event
    }
    const { frequency, interval = 1, count, until, byDay, byMonthDay, byMonth, } = event.recurrence;
    ...
    const matchesMonthDay = (date, byMonthDay) => {
        if (!byMonthDay || byMonthDay.length === 0)
            return true;
        return byMonthDay.includes(date.getDate());
    };
    const matchesMonth = (date, byMonth) => {
    ...
    let instanceCount = 0;
    while (current.getTime() <= cutoffDate &&
 ...
 */
export const getCalendarViewDateRange = (viewDate, viewType) => {
 ...
 */
export const formatDateRange = (startTime, endTime, allDay) => {
 ...
 */
export const countEventsInTimeRange = (events, startDate, endDate, includeRecurrences = true) => {
    if (!events.length)
        return 0;
    // Count regular events first
    const regularEvents = events.filter((event) => event.startTime <= endDate && event.endTime >= startDate);
