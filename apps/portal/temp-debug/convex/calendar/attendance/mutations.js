...
export const updateAttendanceStatus = mutation({
    args: {
        eventId: v.id("events"),
        status: v.union(v.literal("accepted"), v.literal("declined"), v.literal("tentative"), v.literal("pending"), v.literal("needs-action")),
...
export const addAttendee = mutation({
    args: {
        ...
        status: v.optional(v.union(v.literal("pending"), v.literal("needs-action"))),
    },
    handler: async (ctx, args) => {
        // Get the authenticated user's Convex ID
        const userId = await getAuthenticatedConvexId(ctx);
        ...
        const existingAttendee = await ctx.db
            .query("eventAttendees")
            .withIndex("by_event_user", (q) => q.eq("eventId", args.eventId).eq("userId", args.attendeeId))
            .unique();
        if (existingAttendee) {
            throw new ConvexError({
