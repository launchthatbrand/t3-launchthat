...
import { checkOrganizationLimit, generateInvitationToken, generateOrganizationSlug, grantCustomerAccess, validateInvitation, verifyOrganizationAccess, } from "./helpers";
/**
...
export const inviteUser = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await verifyOrganizationAccess(ctx, args.organizationId, userId, ["owner", "admin"]);
        ...
        const existingUser = await ctx.db
            ...
            .unique();
        if (existingUser) {
            const isAlreadyMember = existingMember.some((m) => m.userId === existingUser._id);
            if (isAlreadyMember) {
...
export const removeUser = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (currentUserId !== args.userId) {
            await verifyOrganizationAccess(ctx, args.organizationId, currentUserId, ["owner", "admin"]);
        }
        // Find membership
...
export const grantCustomerAccessMutation = mutation({
    args: {
        ...
        sourceId: v.optional(v.union(v.id("products"), v.id("courses"), v.string())),
    ...
    handler: async (ctx, args) => {
        ...
        const currentUserId = identity.subject;
        // Verify admin access
        await verifyOrganizationAccess(ctx, args.organizationId, currentUserId, ["owner", "admin", "editor"]);
...
export const updatePlan = mutation({
    args: {
        planId: v.id("plans"),
        name: v.optional(v.union(v.literal("free"), v.literal("starter"), v.literal("business"), v.literal("agency"))),
...
export const deletePlan = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (usersOnPlan.length > 0) {
            throw new Error(`Cannot delete plan. ${usersOnPlan.length} user(s) are currently assigned to this plan.`);
        }
        ...
        if (orgsOnPlan.length > 0) {
            throw new Error(`Cannot delete plan. ${orgsOnPlan.length} organization(s) are currently using this plan.`);
        }
