 ...
 */
export const verifyOrganizationAccess = async (ctx, organizationId, userId, requiredRoles) => {
    ...
    if (requiredRoles && !requiredRoles.includes(membership.role)) {
        throw new Error(`Access denied: User role '${membership.role}' insufficient. Required: ${requiredRoles.join(", ")}`);
    }
    return membership;
};
/**
 ...
 */
export const getUserOrganizations = async (ctx, userId, activeOnly = true) => {
 ...
 */
export const checkOrganizationLimit = async (ctx, userId) => {
    ...
    const userOrgs = await ctx.db
        .query("userOrganizations")
        .withIndex("by_user", (q) => q.eq("userId", userId))
        .filter((q) => q.and(q.eq(q.field("isActive"), true), q.eq(q.field("role"), "owner")))
        .collect();
    if (plan.maxOrganizations !== -1 &&
        userOrgs.length >= plan.maxOrganizations) {
        throw new Error(`Organization limit reached. Your ${plan.displayName} plan allows ${plan.maxOrganizations} organization${plan.maxOrganizations === 1 ? "" : "s"}.`);
    }
    return {
        canCreate: true,
 ...
 */
export const getUserPlan = async (ctx, userId) => {
    const user = await ctx.db.get(userId);
    if (!user?.planId) {
        return null;
    }
 ...
 */
export const generateOrganizationSlug = async (ctx, name) => {
 ...
 */
export const validateInvitation = async (ctx, token) => {
 ...
 */
export const getOrganizationBySlug = async (ctx, slug) => {
    return await ctx.db
        .query("organizations")
        .withIndex("by_slug", (q) => q.eq("slug", slug))
        .unique();
 ...
 */
export const grantCustomerAccess = async (ctx, args) => {
    // Check if user already has access
    const existing = await ctx.db
        .query("userOrganizations")
        .withIndex("by_user_organization", (q) => q
 ...
 */
export const isOrganizationAdmin = (userRole) => {
 ...
 */
export const isOrganizationActive = (subscriptionStatus) => {
