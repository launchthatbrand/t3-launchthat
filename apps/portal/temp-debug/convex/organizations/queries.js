...
import { getOrganizationBySlug, getUserOrganizations, getUserPlan, verifyOrganizationAccess, } from "./helpers";
import { organizationInvitationValidator, organizationMemberValidator, organizationValidator, planLimitResultValidator, planValidator, } from "./types";
/**
...
export const searchOrganizations = query({
    ...
    handler: async (ctx, args) => {
        ...
        const organizations = await ctx.db
            .query("organizations")
            .filter((q) => q.or(q.eq(q.field("isPublic"), true), q.eq(q.field("ownerId"), user._id)))
            .take(limit);
        // Filter by search query
        return organizations.filter((org) => org.name.toLowerCase().includes(args.query.toLowerCase()) ||
