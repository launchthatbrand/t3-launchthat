...
export const syncVimeoVideos = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        const connection = await ctx.runQuery(internal.integrations.connections.get, {
        ...
        const response = await fetch("https://api.vimeo.com/me/videos?fields=uri,name,description,link,created_time,pictures.sizes&page=1&per_page=100", {
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to fetch Vimeo videos: ${response.status} ${text}`);
        }
        ...
        for (const video of data.data) {
            ...
            const existing = await ctx.runQuery(internal.vimeo.queries.getVideoByExternalId, {
                videoId,
            });
            if (existing) {
...
export const syncAllConnections = internalAction({
    args: {},
    handler: async (ctx) => {
        const vimeoApp = await ctx.runQuery(internal.integrations.connections.queries.getVimeoApp, {});
        if (!vimeoApp)
            return;
...
export const listFolders = action({
    ...
    handler: async (ctx, args) => {
        // Fetch connection
        const connection = await ctx.runQuery(internal.integrations.connections.queries.get, { id: args.connectionId });
        if (!connection)
            throw new Error("Connection not found");
        // Parse credentials
        let creds = {};
        ...
        if (!res.ok)
            throw new Error(`Vimeo API error ${res.status}`);
        const json = (await res.json());
        const folders = json.data?.map((d) => ({
...
export const fetchVimeoVideos = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        const vimeoApp = await ctx.runQuery(internal.integrations.connections.queries.getVimeoApp, {});
        if (!vimeoApp)
            throw new Error("Vimeo app not found");
        // 2. Find the connection for this user/org
        const connection = await ctx.runQuery(internal["integrations/connections"].queries.getConnectionByAppAndOwner, { appId: vimeoApp._id, ownerId: args.ownerId });
        ...
        const response = await fetch("https://api.vimeo.com/me/videos?fields=id,uri,name,folder,description,link,created_time,pictures.sizes&page=1&per_page=100", {
            headers: {
                Authorization: `bearer ${accessToken}`,
                Accept: "application/vnd.vimeo.*+json;version=3.4",
            },
